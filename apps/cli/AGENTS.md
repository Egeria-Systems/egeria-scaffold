...
- `plan-add` remains read-only and is limited to `booking-calendly` with paired validated Calendly settings. Require builder-core's clean attached linked worktree preflight before and after planning, reject hidden tracked state and ignored create targets, contain unexpected reader/Git failures under their stable codes, return only the redacted plan or refusal code, and never create the worktree or branch. Do not transform, migrate, persist state, install dependencies, or contact a provider.
