...
export function Web3FormsContact({ settings, content, locale }: Readonly<{
  ...
  const clearError = useCallback((field: keyof ContactErrors) => {
    setErrors(current => {
  ...
  const receiveToken = useCallback((token: string) => {
    ...
    if (token.trim() !== "") clearError("captcha");
  }, [clearError]);
  useEffect(() => () => {
  ...
  return <ContactForm content={content} fields={fields} errors={errors} pending={pending} status={status} formRef={formRef} statusRef={statusRef} onChange={(field, value) => { setFields(current => ({ ...current, [field]: value })); clearError(field); } } onSubmit={event => { void submit(event); } } captcha={
    <>
