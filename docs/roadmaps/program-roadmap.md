...
**Development stream:** Automatic-CI Plan A runs only in isolated branch/worktree `ci-efficiency-security` from accepted `main@ee1e1df10fa2be2f09333efecd86de7f7a131d49`. Task 6D and its accepted ancestry repairs are ancestors; the separate `standards-certification` branch remains stopped and Plan A does not modify it. If Plan A integrates, renewed affected Task 6D evidence occurs only when that separate stream resumes on a descendant of the accepted Plan A revision. Separate final-diff approvals remain mandatory. This does not alter the isolated-worktree requirement for builder commands that transform generated client repositories.
...
**Current capability-certification tasks:** P2 Task 6B is the separate `observability@0.2.0` certification increment. Its [fresh-scaffold runner](../../scripts/certify-production-observability.mjs) passed the reviewed local outcome, while the [manual protected-staging workflow](../../.github/workflows/production-observability-certification.yml) and [provider receipt template](../implementation-evidence/production-observability-provider-receipt-template.md) are prepared but unexecuted. Accepted `main` records the exact `standards@0.3.0` subject as certified through the [subject-bound local receipt](../implementation-evidence/2026-08-12-generated-unit-component-testing-certification-verification.md), with the reviewed rerun receipt bound to accepted-main revision `c9294e9dc59d4b7bafed406846af3b43a10733d3` by accepted repair `ee1e1df10fa2be2f09333efecd86de7f7a131d49`. The strict [`certifications/capabilities.json`](../../certifications/capabilities.json) registry admits standards as `certified` and observability as ordinary `pending`. Plan A does not change that subject or transition; its later operational bytes require the separately resumed Task 6D evidence renewal described above. Within the remaining observability task, protected-staging, provider/source, credentials, telemetry transmission, cleanup, certification transition, merge, and push remain unauthorized; transition and all-certified closure correctly remain open.
