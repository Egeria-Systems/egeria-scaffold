...
**Current capability-certification task:** P2 Task 6B is the separate `observability@0.2.0` certification increment. Its [fresh-scaffold runner](../../scripts/certify-production-observability.mjs) passed the reviewed local outcome, while the [manual protected-staging workflow](../../.github/workflows/production-observability-certification.yml) and [provider receipt template](../implementation-evidence/production-observability-provider-receipt-template.md) are prepared but unexecuted. The strict [`certifications/capabilities.json`](../../certifications/capabilities.json) registry admits its exact descriptor version/digest as ordinary `pending` and links the present certification plan. Protected-staging, provider/source, credentials, telemetry transmission, cleanup, registry transition, merge, and push remain separately unauthorized; transition and all-certified closure correctly remain open.
