...
**Development stream:** Automatic-CI Plan A is integrated at accepted `main@368b9491fd2f813f83f1e456823d8c7546f6762c`. Task 6D and its accepted ancestry repairs are ancestors, but the separate standards-certification stream still owns renewed affected evidence on a descendant of that accepted revision. The observability certification transition remains on `observability-error-diagnostics` until separately integrated. Separate final-diff and integration approvals remain mandatory. This does not alter the isolated-worktree requirement for builder commands that transform generated client repositories.
...
**Current capability-certification tasks:** P2 Task 6B is the separate `observability@0.2.0` certification increment, and its local certification transition is complete. The [successful protected-staging receipt](../implementation-evidence/2026-08-12-production-observability-certification-provider-receipt.md) binds the reviewed `fresh-scaffold` and `deployed-application` outcomes to revision `ee1e1df10fa2be2f09333efecd86de7f7a131d49`. `cleanup-recovery` is not claimed; the retained-resource disposition is accepted by explicit user decision and remains operationally uncertified. The strict [`certifications/capabilities.json`](../../certifications/capabilities.json) registry admits observability as `certified` in this candidate, so admission and `legacy-backfill-exempt` transition closure pass while `all-certified` still rejects the four frozen backfills. Diagnostics Task 5 is selected as the next increment only after this certification transition is integrated, the post-Plan-A standards evidence renewal is integrated, and every remaining Gate B preflight succeeds. This transition does not authorize push, merge, package publication, another workflow, provider mutation, cleanup, or production action.
