...
Calendly account/event configuration, provider scheduling data, cookies, retention, and cleanup are unmanaged. Local and stubbed-browser results do not establish provider availability, a real booking, hosted execution, live deployment, visual quality, human accessibility, production readiness, or WCAG conformance. Source rollback is separate from provider cleanup. Protected-staging deployment, a synthetic booking, provider-confirmed success, and provider cleanup remain unexecuted and require separate explicit authorization.
...
**Current capability-certification task:** P2 Task 5B owns the separate `booking-calendly` certification increment. Its dated preparation and exact-file plan use current official sources for account, service-tier, credential, protected-staging, synthetic-data, cost, cleanup, and recovery requirements. The repository now has the strict [`certifications/capabilities.json`](../../certifications/capabilities.json) coverage registry, actual admission and closure gates, evidence-revision ancestor and completed-review receipt checks, and an actual compiled-CLI fresh-scaffold runner that reuses the generated-project verifier. `booking-calendly` remains pending: protected-staging deployment, provider-confirmed booking, cancellation, and cleanup are unexecuted and separately authorized. This task introduces no new provider outcome scenario and does not authorize deployment, provider configuration, credentials, spending, or another external action.
