...
**Current capability-certification task:** P2 Task 6B is the separate `observability@0.2.0` certification increment. Local certification work is authorized and in progress with reviewed local fresh-scaffold evidence. The strict [`certifications/capabilities.json`](../../certifications/capabilities.json) registry admits its exact descriptor version/digest as ordinary `pending` and links the present certification plan. Protected-staging, provider/source, credentials, telemetry transmission, cleanup, registry transition, merge, and push remain separately unauthorized; transition and all-certified closure correctly remain open.
