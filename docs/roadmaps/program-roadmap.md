...
**Development stream:** Task 6C remains on its approved isolated merge-request branch/worktree and has reconciled accepted `main@2a315aa0e7dce1bf1048b9a2c07e318add9241de` under the [review and contribution protocol](../governance/review-and-contribution.md). It does not independently implement another certification increment. This does not alter the isolated-worktree requirement for builder commands that transform generated client repositories.

**Current capability-certification tasks:** P2 Task 6B is the separate `observability@0.2.0` certification increment. Its [fresh-scaffold runner](../../scripts/certify-production-observability.mjs) passed the reviewed local outcome, while the [manual protected-staging workflow](../../.github/workflows/production-observability-certification.yml) and [provider receipt template](../implementation-evidence/production-observability-provider-receipt-template.md) are prepared but unexecuted. Task 6C also replaces legacy standards coverage with an ordinary `standards@0.3.0` pending subject linked to the separate Task 6D plan; it has no certification evidence yet. The strict [`certifications/capabilities.json`](../../certifications/capabilities.json) registry admits both exact descriptor versions/digests as ordinary `pending`. Within those separate capability-certification tasks, protected-staging, provider/source, credentials, telemetry transmission, cleanup, certification transition, merge, and push remain unauthorized; transition and all-certified closure correctly remain open.
