...
**Current capability-certification task:** P2 Task 5B owns the separate `booking-calendly` certification increment. Its dated preparation and exact-file plan use current official sources for account, service-tier, credential, protected-staging, synthetic-data, cost, cleanup, and recovery requirements. The repository now has the strict [`certifications/capabilities.json`](../../certifications/capabilities.json) coverage registry, actual admission and closure gates, and an actual compiled-CLI fresh-scaffold runner that reuses the generated-project verifier. `booking-calendly` remains pending: protected-staging deployment, provider-confirmed booking, cancellation, and cleanup are unexecuted and separately authorized. This task introduces no new provider outcome scenario and does not authorize deployment, provider configuration, credentials, spending, or another external action.
