...
**Current capability-certification task:** P2 Task 6B is the separate `observability@0.2.0` certification increment. The strict [`certifications/capabilities.json`](../../certifications/capabilities.json) registry admits its exact descriptor version/digest as ordinary `pending`, with reviewed local fresh-scaffold evidence, and links the present certification plan. Protected-staging, provider, and cleanup evidence remain absent; transition and all-certified closure correctly remain open. Executing that plan requires separate approval and any deployment, provider/source configuration, credential change, spending, telemetry transmission, cleanup, or other external action requires its own current authority.
