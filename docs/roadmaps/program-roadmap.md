...
**Current capability-certification tasks:** P2 Task 6B is the separate `observability@0.2.0` certification increment, and its local certification transition is complete. A private protected-staging receipt binds the reviewed `fresh-scaffold` and `deployed-application` outcomes to revision `ee1e1df10fa2be2f09333efecd86de7f7a131d49`. `cleanup-recovery` is not claimed; the retained-resource disposition is accepted by explicit user decision and remains operationally uncertified. The exact `standards@0.3.0` subject remains `certified`, with its renewed all-eight-outcome evidence integrated on accepted `main`. The strict [`certifications/capabilities.json`](../../certifications/capabilities.json) registry admits observability as `certified` in this candidate, so admission and `legacy-backfill-exempt` transition closure pass while `all-certified` rejects the four frozen backfills. Diagnostics Task 5 is selected as the next increment only after this certification transition is integrated and every remaining Gate B preflight succeeds. This transition does not authorize merge, package publication, another workflow, provider mutation, cleanup, or production action.
