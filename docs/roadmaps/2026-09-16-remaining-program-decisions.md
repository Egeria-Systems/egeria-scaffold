...
## Contact and portability scheduling decision

**Recorded 2026-09-24:** The user defers P5F and P10-4 while keeping every other increment. The [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred) owns scope, completion and reactivation. D5's contact requirements below remain preserved for later use; only the named portability experiment and its contingent package/support decisions in D11 are deferred. D6–D10, the complete P7 recipe and the P7-only composition exception are unchanged. Retained site-payment persistence is assigned to P8C-1 with separate exact-subject acceptance and no new certification exception.
...
| **D5** | Recommended approach plus best practice. | Minimal durable contact with abuse/privacy controls and optional notifications. Use bounded, documented policy settings; no attachments or arbitrary form engine. Concrete policy defaults are prepared at P5F's gate if separately reactivated under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred). |
...
| **D11** | Recommended approach; performance still deferred. | Observed-risk fleet selection, real supported version edges, bounded portability and explicit recovery remain the recorded requirements. The later [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred) defers only the P10-4 alternate-adapter experiment and its contingent support/extraction decisions; retained fleet/package reassessment continues. Performance measurement/budgets remain deferred; this task does not reopen them. |
