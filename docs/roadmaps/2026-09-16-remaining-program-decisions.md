...
D1/D2 are answered. D3 delegates a bounded evidence-led engineering recommendation. D4 requires its planned topology proof; D12's staging direction is now settled, with canonical application and exact implementation gates still required. D5–D10 delegate reasonable defaults and useful configurability, not unspecified material policy or arbitrary scope expansion. D11 still needs actual authorized fleet targets at execution time; no further program-level performance decision is required.
