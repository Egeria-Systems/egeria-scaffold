...
**Status:** Approved program direction from 2026-09-16, reconciled with integrated delivery on 2026-09-23. Environment requirements reconciled on 2026-09-24 under [ADR-0020](../adr/0020-application-environments.md). All 28 original increment cards retain their identifiers and requirements; integrated cards are history, and five implementation cards (P5F-1–4 and P10-4) are deferred under the 2026-09-24 [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred). They are not 28 remaining active tasks. The P5D implementation cards are historical requirements for the integrated Resend delivery. Remaining exact plans, final diffs, certification and external actions retain their separate gates.
...
**Active scope:** P5D, P5E, P6, P7, P8A, P8B, P8C, P9 and P10-1/2/3/5. P5F and its contact certification tasks, plus P10-4, are preserved as deferred requirements under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred). P5C is an existing predecessor stream, not a new assignment. P5A and P5B retain their relocation to P3B.
...
**Existing acceptance-record conflict:** persistence is integrated, while the roadmap still records P4 as open and requires its acceptance before persistence integration. The owning closeout must reconcile those facts before downstream runtime admission. This planning publication infers no waiver and changes no historical status or evidence. The later [environment admission](program-roadmap.md#lean-application-environments) is a separate new bounded stream; it does not resolve this historical acceptance gap or waive unrelated downstream gates.

### Current delivery position

The [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred) qualifies current scheduling below without rewriting integrated history or earlier one-time authority. No other increment, pending certification or environment-stream obligation is removed.

**Environment handoff, 2026-09-24:** Use the exact baseline and pending-subject inventory in the [environment stream](program-roadmap.md#lean-application-environments). Preserve P5D-1 contract, P5D-2 sender and P5D-3 lifecycle bodies as historical implementation, reconciling new target/recipient behavior through environment card 7 and its guide; P5D-C renews actual changed subjects and supported lifecycle. P5E-1 remains historical local topology evidence, including its acknowledgement/retry limitation. P5E-2 runtime is present through merged PR #146 at `fb16c9128d8e461f52a8ad6a2bb4b1e7a368d566`; merge is not retrospective final acceptance or certification. ADR-0019 owns current primary-Worker behavior. Environment cards 2/9 reconcile target/deployment and jobs guidance; jobs lifecycle/replay/drain remain refused until P5E-3. The older “candidate through an open MR” wording below records its original authority boundary, not a second runtime assignment.
...
**Current environment amendment:** The common target/configuration and scoped retirement decision is accepted in ADR-0020; executable environment gates remain planned. Existing email/persistence/jobs detailed handoffs were inspected as exact historical-subject evidence, not active permission to retain contradictory settings. The roadmap admission records the audit gaps. Each later certification must bind its actual subject, target, runbook and authority before use.
...
[ADR-0020](../adr/0020-application-environments.md#generation-and-precise-supersessions) qualifies references to continued executable historical compatibility below only for the accepted unused-generation retirement. Preserve current public behavior during assembly and historical receipts afterward; later work names real supported new-generation tuples. Do not invent old-to-new migrations or restore a retired generation for a fixture. The [scoped protocol exception](../governance/review-and-contribution.md#lean-application-environments-delivery-exception) applies only to the environment stream, not these other future gates.
...
### Application environment acceptance

[ADR-0020](../adr/0020-application-environments.md) is the common contract for every newly implemented or materially changed service. At first executable delivery, define development, restricted staging and production: fixed public build inputs, runtime secrets, local simulations, hosted resource identities, origins/callbacks, delivery destinations and target-specific state/recovery. Do not use NODE_ENV as the release target or fall back to production when nonproduction configuration is absent. Staging uses production compilation and Access protection; production is separately built from the same reviewed source revision. Ordinary local/CI tests use synthetic data and controlled dependencies. Local, hosted, provider-confirmed and recovery evidence remain distinct; an identifier is not proof of resource ownership.

Share nonproduction resources where appropriate; justify finer isolation with an actual provider rule, state/security boundary or test-interference reason. Google's deployment-environment projects are an explicit exception. Manual development/staging may share a general Stripe sandbox; separately authorized live-provider CI uses a dedicated sandbox, while ordinary CI needs no provisioning. Refresh relevant official provider guidance at implementation, preserving these distinctions between provider requirements and project policy.

Every card's owning guide includes exact names/locations, public versus secret and build versus runtime mapping, provider prerequisites/tier limits, permitted sharing, actual local/staging/production commands with placeholders, expected destinations/results, common failures, verification, disabling and recovery that retains reusable resources. Include the consuming composed example below, linking shared/dependency guidance instead of duplicating it. Field 5's concrete checks and the certification row verify the advertised combination: target/success, allowed optional absence, missing required dependency refusal, meaningful failure and recovery. These owner/test/checkpoint duties arrive with the capability; P10 only checks consistency and measured gaps. Do not create future runtime, setup tooling or a Cartesian test matrix now.

**Provider callbacks:** At the owning implementation gate, define one exact signed-webhook method/path exception or equivalently narrow ingress through otherwise protected staging. Verify raw-body signature, expected account/subscription/environment, replay window, request-size/rate bounds and durable receipt requirements before effects. Never exempt `/api/*`, embed service tokens in URLs, remove application authorization or bypass CAPTCHA. Browser OAuth/Checkout returns continue through the user's Access session. Provisioning any future exception requires separate authorization; a bypass removes Access protections and is not proof of application safety.

**Stateful target changes:** Migration, seed/reset, import/export, account deletion, drain/replay, webhook replay, restore and provider cleanup require positive target identity and their applicable data/recovery approval. Use local synthetic defaults, stable staging resources and per-target concurrency where effects conflict. Never automatically copy production data to nonproduction. Credentials/data are not promoted with code; source rollback is not data or provider rollback.

**Named composed acceptance:** Current rows complete in the environment stream; future rows complete with their consuming capability. The same scenario may provide explicit assertions for several exact subjects, never a composite certificate. Preserve independent selections and actual descriptor dependencies.

| Flow | Delivery and guide/example owner | Observable acceptance and evidence limits |
| --- | --- | --- |
| Current public contact + booking + analytics + multilingual | Environment cards 3–5 own service guides; 9 owns deployment use; 10 reconciles complete example | Local absence produces no requests; analytics-off leaves contact/booking functional; consent controls/locale and optional removal preserve behavior; configured isolated staging/Access verified by controlled browser tests. Real provider confirmation remains separate. |
| Current foundation + independently optional persistence/email/jobs | Cards 2/6/7 own shared/server guides, 9 jobs deployment guide, 10 composed example | Existing health and synthetic provider-neutral ports exercise matching target and positive outcomes; absent optionals remain absent; required binding/recipient/queue mismatch fails before effects, and unsupported jobs lifecycle edges refuse under the [jobs boundary](../architecture/capability-model.md#background-job-delivery-boundary). No invented CRUD or business flow. |
| Durable contact → DB → optional email/jobs | **Deferred:** P5F-1–4 and contact consuming guide; C0/CP/C, under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred) | No-email/direct/queued flows, post-commit event, DB failure, notification outage, lost enqueue/duplicates and safe retry; stage Turnstile/provider journey separately authorized. |
| CMS → DB/media → multilingual | P6-1–3, editorial/import/cutover guide; P6-C | Both install orders, locale/draft preview behind Access, denied known-media access, paired import/export/restore and wrong-target refusal; live cutover separately approved. |
| Identity → DB/email → protected area/account/support | P7-1–3, identity/account/support consuming guide; shared CF, CI/CP/CG/CA/CS/final CF/CL | Synthetic sign-up/verify/session/Google link and role-bound CRUD/support, exact target links, Access-vs-app negative cases, recovery/cleanup; actual provider and deployed security claims need their own proof. |
| Identity → optional TOTP/passkeys | P8A-1/P8B-1, factor/passkey consuming guides; A/B certification | Simpler core identity when absent; required core refusal, real approved assurance, environment-bound enrollment/login/lost factor/recovery; virtual authenticators do not prove physical-device coverage. |
| Payments → persistence/declared jobs → billing subject/signed ingress | P8C-1–3, including payment-owned site persistence admission and separate exact-subject acceptance; payment consuming guide; P8C-C | Preserve declared dependency graph, one-time without identity and subscription real billing consumer, exact endpoint/sandbox target, duplicates/out-of-order/refund/cancel/failure/recovery; no live-money tests. |
| Booking webhooks → durable receipt → optional jobs | P9-1–2, webhook consuming guide; P9-C | Direct/queued signed booking/cancellation with target/account/subscription checks, receipt/enqueue failure/replay recovery; booking embed never supplies this assertion. |
...
The [environment acceptance contract](#application-environment-acceptance) adds target, guide/example, composed-flow, local-negative and separate hosted/recovery assertions to every affected requirement below. Concrete additions are in the original owning cards and §7 rows; dependencies, policy decisions and certification identities are unchanged.
...
| Site dependency-only persistence required by retained payments; capability catalog and existing persistence contract | P8C-1 owns payment-consumed site admission and its executable certification sequence | Separate exact persistence/shared-subject acceptance before site payment consumption; no contact prerequisite, P5C restart or certification exception |
| Public-profile persistence widening for contact; capability catalog | **Deferred:** P5F-2 under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred); reuse actual accepted support on reactivation | Deferred P5F-CP; portfolio contact widening remains deferred |
| Durable accepted contact before success; Turnstile, limits, status, retention/deletion, no body logs; source §13 | **Deferred:** P5F-1; public-profile extension P5F-2 | Core test/abuse/data gate; P5F-C0, P5F-CP and final P5F-C |
| Stable contact application event for optional integrations; source §13 | **Deferred:** P5F-1 defines the minimal post-commit event; P5F-3/4 consume it | No pre-commit emission or sensitive payload; P5F-C0 and P5F-C |
| Contact email optional, queue optional, notification failure cannot undo acceptance; source §3/13 | **Deferred:** P5F-3/4 | Persistence/send/enqueue crash boundaries; P5F-C |
...
| Fleet measurements, package reassessment, evidence-backed simplification and selected real-fleet certification; source §21/P10 | P10-1–3 retained; P10-4 alternate-adapter experiment and contingent decisions **deferred** under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred) | Retained decisions and affected certification renewals; amended active-scope final approval |
...
4. P6-1 → P6-2 → P6-3 → P6-C.
5. P7-1 → accepted canonical staging amendment and exact Gate 2 → P7-2 complete fresh baseline → prerequisite shared-subject certification within P7-CF → P7-CI → P7-CP/CG/CA after their actual dependencies → P7-CS after CI/CP/CA → P7-CF final composed-baseline checkpoint → P7-3 migration → P7-CL lifecycle renewal → explicit P7 closure. The user approved only this initial-composition implementation start-order exception. Section 7 makes the shared-subject and identity certification order explicit; all other gates remain.
6. P8A-1 → P8A-C; P8B-1 → P8B-C; P8C-1 → P8C-2 → P8C-3 → P8C-C. Their independent readiness is shown below; the serial ordering is a low-conflict default.
7. P9-1 → P9-2 → P9-C.
8. P10-1 → P10-2's evidence-selected repairs and separate renewals → P10-3 → P10-5 → final acceptance of the amended active scope.

**Deferred sequence, not an active assignment:** P5F-1 → P5F-C0 → P5F-2 → P5F-CP → P5F-3 → P5F-4 → P5F-C retains its requirements for reactivation and reconciliation with actually accepted persistence support. P10-4 retains its evidence-led experiment and contingent decisions. Both follow the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred); neither blocks retained payments, fleet work or P10-5. P8C-1 owns the site persistence prerequisite and strict admission gate below.
...
flowchart TD
  ...
  P4 --> PAY[P8C app one-time]
  PC --> PAY
  EC --> PAY
  PC --> SITE[Payment-owned site persistence admission and separate certification]
  SITE --> SITEPAY[P8C site one-time]
  P4 --> SITEPAY
  EC --> SITEPAY
  ...
  PAYC --> FLEET
  WHC --> FLEET
  FLEET --> RECOVERY[P10-3 selected recovery evidence]
  RECOVERY --> DOC[P10-5 documentation hardening and active-scope acceptance]
```

The site persistence node is a P8C-1 preparation/admission gate, not a new increment or a dependency of already-supported app persistence. Its exact executable sequence still requires future approval; unsupported site selection stays closed. Deferred contact and portability are outside this active graph.
...
2. **Predecessor:** Accepted P5E-2, merged MR #146 at `fb16c9128d8e461f52a8ad6a2bb4b1e7a368d566`, under the prospective [lifecycle/operator exception](program-roadmap.md#one-time-2026-09-24-job-lifecycle-and-operator-safety-exception).
3. **Owners:** Builder job lifecycle, reference guard, deployment topology, generated operator commands and consuming handler contracts. Own the jobs operator guide and exact replay/drain worked example.
...
5. **Tests / recovery:** L plus producer-without-consumer refusal, consumer version/schema mismatch, replay authorization, duplicate replay, partial drain, stale disposition, missing resource, queue failure after source mutation and operator diagnostics. Recovery distinguishes repository prefix, deployed consumer and queued messages. Environment acceptance: Use local simulated queues without remote credentials or wrangler dev --remote. Bind later permission and recovery to the exact environment, queue, consumer/dead-letter target and handler/schema. Wrong-target operations refuse before dispatch/replay; preserve six identities, retention and nonlocal attestation, which is not remote verification. Carry forward the topology proof’s local acknowledgement/retry limits and distinguish authorized hosted retry/terminal/retention evidence.
...
**Deferred in full:** P5F-1–4 and P5F-C0/CP/C follow the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred). Imperative card text below preserves requirements if reactivated, not currently scheduled or authorized work. Contact-specific guides, composed flows, fixtures and views are likewise deferred. Retained site payments use P8C-1's persistence gate, not contact certification.
...
5. **Tests / recovery:** G/L, streaming oversize bodies, token expiration/replay/wrong hostname, challenge-service failure, rate-limit races, malformed input, D1 failure, duplicate retry after lost response, unauthorized read/export/delete, retention and restore/readback. Verify no event before commit, stable event identity and no sensitive event/log payload. Before-commit failure has no success; notification consumers are absent. Environment acceptance: Ordinary CI intercepts Siteverify with paired documented dummy success/rejection/spent-token fixtures; any real dummy-key network check is separately declared. Staging and production use distinct real widget/secret pairs and exact hostname/action checks; reject known dummy keys in release preflight and never bypass missing validation. Wrong host/replay fails before persistence; missing or misbound APP_DB fails before write. Use synthetic intake, same-target dependencies and a real authorized staging challenge/cleanup journey for provider claims; Web3Forms hCaptcha is a separate contract.
...
On reactivation, discover and reuse the exact persistence support already accepted through payments or another authorized consumer. This preserved card does not restart completed widening; portfolio contact support remains deferred with it.
...
3. **Owners:** Existing persistence and contact snapshot/admission owners, generated foundation/runtime verification, resolver/locks, deployment bindings, CLI and actual lifecycle. Preserve P5C's accepted local-first/export-removal choices; no business-table or runner redesign. Own the public contact/persistence composed guide example.
...
5. **Tests / recovery:** Fresh portfolio/site contact and compiled existing-project addition; multilingual/content/route preservation; exact dependency inference/locks; APP_DB identity and wrong-target refusal; partial failure, reference-protected removal, source/export/restore and database loss boundaries. Exercise real public consumers, not a manually forced impossible persistence-only selection. Environment acceptance: Prove contact and APP_DB select the same environment on supported public profiles, wrong-target binding/export/restore refuses and absent optional notifications preserve usable intake. Certification uses the actual contact/persistence pair, not an invented persistence-only public baseline.
6. **Commit / review / checkpoint:** One consumer-driven compatibility-extension scope with R. Separate P5F-CP tasks certify the changed persistence and contact subjects and any other material shared subject before optional notification work. Retained site-payment prerequisites now belong to P8C-1.
...
3. **Owners:** Contact notification use case/template/status, its stable event and email sender adapter; the contact domain's acceptance contract is unchanged. Own the direct-notification consuming guide example.
...
5. **Tests / recovery:** Persist/event/send order, rejection/timeout/unknown acceptance, repeat submission, repeated notification attempt, missing configuration, localized content and private logs. Operator retry requires authorization and acknowledges provider idempotency expiry; no claim of exactly-once delivery. Environment acceptance: Use the same target for persisted submission and optional sender/domain/recipient policy. Misbound or non-allowlisted email refuses without undoing accepted intake; distinguish missing required persistence from valid absence of optional email. Notification outage and recovery remain synthetic in ordinary tests.
...
3. **Owners:** Contact-owned outbox/delivery state and repository transaction; jobs transport/handler; sender; conditional support-console consumer later. Own the queued-notification guide and commit/enqueue recovery example.
...
5. **Tests / recovery:** Crash after database commit/before enqueue, enqueue accepted/ack lost, worker crash after send, duplicate/out-of-order delivery, lease/retry races, terminal failure, reconciliation restart, deleted/expired submission and poison intent. Review recovery across D1, Queue and Resend independently. Environment acceptance: Bind durable intent, queue/envelope and optional email policy to the same environment. Exercise lost enqueue, repeat delivery, target mismatch and recovery without production fallback. Keep separate no-email, direct-email and queued-email acceptance; optional absence preserves the simpler flow.
...
3. **Owners:** ContentRepository contract at its actual first consuming boundary, Payload/D1/R2 adapters and Cloudflare configuration. Preserve application-customer identity separation. Own the local CMS/DB/media guide and protected-preview example.
...
5. **Tests / recovery:** Actual workerd D1/R2, protected read/write, preview authentication, normalization/locale mapping, unsupported media and separate CMS_DB/APP_DB identities. If selected packages fail, retain evidence and stop; do not broaden platforms silently. Environment acceptance: Use local storage; separate staging/production CMS_DB, R2 bucket, signing/encryption credentials, staff, preview origins and caches, retaining CMS_DB distinct from APP_DB. Choose SQLite development push or manual migrations coherently; never run development push on shared remote storage or mix incompatible flows. Deny known draft/media URLs without authorization, disable public/development R2 bypass paths and never enable Payload access-control bypass to render staging media.
...
3. **Owners:** CMS module and adapters, content schema/normalization, deployment binding surfaces, localized preview UI and media lifecycle. Existing section registry remains the executable composition authority. Own the bilingual editorial/import/parity/export guide example.
...
3. **Owners:** Content adapter selection/composition, builder migration/inference/state, CMS import/export and operator runbook. Payload controls only the approved client-editable editorial content; application/security copy has an explicit owner. Own the cutover/reverse-export and paired-restore guide example.
...
5. **Tests / recovery:** L and G, failed parity, concurrent edit, partial import, stale approval, commit/cutover crash, cache publication/withdrawal, CMS outage, export round-trip, database/media recovery mismatch and both locale installation orders. Environment acceptance: Bind cutover/export/ejection/restore to the exact environment and paired CMS_DB/R2 snapshot. Reject wrong target or mismatched pair before mutation. No automatic production data copy or production cutover follows from successful staging parity; approved editor and recovery identities remain separate.
...
Under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred), conditional contact-delivery views remain dormant: no contact capability means no contact UI or tests claiming contact support. The complete account/support baseline, Google sign-in and later conditional billing views remain required.
...
3. **Owners:** Identity adapter, request/authorization boundary, D1 migration owner and deployment proof. Use current installed contracts; do not silently change Effect, Next or test majors. Own the identity environment/security guide and deployed compatibility example.
...
5. **Tests / recovery:** Deployed verified-email/recovery/session lifecycle, cookie/security headers, trusted origins, CSRF, ownership, enumeration/rate-limit behavior, D1 consistency, credential separation and cleanup. Missing provider/security proof blocks product acceptance. Environment acceptance: Bind auth base URL, trusted origins, database, signing/encryption secrets, cookie scope and verification/recovery links to each environment; never trust arbitrary Host or a production-spanning wildcard. Use separate Google projects/clients per deployment environment, exact redirects and authorized test users. Exercise Access expiry/reauthentication without forwarding service credentials to Google. Existing D7/D8 policy gates remain.
...
3. **Owners:** Distinct identity, Google adapter, protected-area, account-profile and support modules retain their catalog identifiers and consuming ports; their actual D1 tables/migrations, localized pure UI, recipe/catalog/CLI, deployment and tests stay with their owners. No generic identity facade. Implement BillingSubjectProvider only when P8C brings its real consumer. Own the identity consuming guide linking email, protected-area, account and support worked examples.
...
5. **Tests / recovery:** G includes actual complete generation, memory contracts, D1/whole-Worker integration, browser/axe and negative security cases for every capability below. New-directory generation retains exact state-last behavior. Ejection/export/removal policies and recovery runbooks must be usable for every newly stored data type; capability lifecycle that is not yet executable is explicitly refused until P7-3. No supported upgrade edge is invented. Environment acceptance: Core: isolated auth URL/origins/cookies/secrets/database and same-target allowlisted verification/recovery mail with safe rejected-mail outcomes. Google: exact project/client/redirect, sign-in/link/unlink, Access expiry and last-method fallback with secret-free redirects. Protected-area: satisfy Access while missing/stale app session, non-admin and cross-user direct APIs still fail. Account: synthetic users and target-bound export/delete/hooks/resumable cleanup/restore; refuse cross-target mutation and add no uploads. Support: distinct staging admins, visible operator target, exact action/target confirmation and audit, conditional same-target views, no Access-derived privilege or cross-environment dashboard.
...
3. **Owners:** Existing operation-specific transition and capability lifecycle policy, installed snapshots, CLI, identity/account migrations and ownership, recipe and state/lock control. Every capability keeps its own removal policy. Own the incoming migration and account recovery guide examples.
...
5. **Tests / recovery:** L/G with actual compiled app transition, representative optional subsets, fresh versus migrated equivalence, partial schema setup, drift/ejection/custom-content refusal, failed migration/export, exact state-last persistence, retained prefix, no-data-loss claims bounded to actual evidence. Exercise reviewed source removal, profile export-and-remove, identity ejection and unsupported repeats/edges. Environment acceptance: Use only genuinely supported new-generation source/target contracts; unknown/mixed or wrong-target state refuses before writes. Bind DB/email, link/unlink, account cleanup hooks, export/restore and support operations to the selected environment. Test partial recovery and preserved optional behavior without resurrecting retired generations.
...
3. **Owners:** Identity assurance policy, library adapter, factor data migrations, protected session/use cases, localized UI and capability lifecycle. Own the TOTP enrollment/lost-factor/reset guide example.
...
5. **Tests / recovery:** Wrong/replayed/expired code, clock window, brute force, concurrent recovery-code use, stale enrollment, lost device, disable/re-enroll, recovery-secret rotation and all installed login-method paths. G/L plus protected staging and actual recovery. Environment acceptance: Use separate synthetic accounts, factor records, encryption keys and recovery material per environment, never production seeds/codes in fixtures or logs. Cross-environment recovery data cannot consume another target’s record, and staging reset cannot affect production. Preserve real attempt/reauthentication/verification policy: credential-signin TOTP alone does not prove OAuth/passkey assurance under D8.
...
3. **Owners:** WebAuthn/library adapter, credential and challenge storage, session authorization, browser UI and lifecycle/migrations. Own the passkey registration/login/recovery guide example.
...
5. **Tests / recovery:** Browser virtual-authenticator cases, wrong origin/RP/challenge, replay, concurrent consume, rejected user verification, revoked credential, lost device, last-credential removal and fallback interactions with password/Google/TOTP. Actual-device/human evidence is required only where the chosen claim cannot be established by automation; record limits. Environment acceptance: Use a local RP/origin accepted by the chosen maintained library, distinct staging/production RP/origins and credential stores. Do not widen production trust or assume copied RP credentials work. Test after Access admission, challenge expiry during reauthentication, wrong RP/origin/challenge and cross-target credentials, revocation and recovery. Virtual-authenticator results and separately authorized real-device claims remain distinct; RP/domain changes are explicit compatibility events.
...
2. **Predecessor:** Certified app foundation, persistence and jobs; D9 one-time subject/entitlement and refund policy. P7 and contact are not required for one-time mode. P8C-1 preparation owns site dependency-only persistence admission: name the real supported site payment consumer, predecessor acceptance, and an executable sequence for the shared extension and its separate exact-subject certification before admitting dependent site-payment implementation. Unsupported site selection stays closed until that gate is met. If joint initial composition needs an exception, obtain a separately approved exact future plan; this amendment grants none. Retain the site/app outcome and P5C's accepted app baseline without restarting persistence.
3. **Owners:** Billing application/domain, BillingGateway/Stripe adapter, APP_DB receipt/payment/entitlement repositories, Queue handler, hosted checkout delivery and builder mode/lifecycle. This workstream also owns the payment-consumed site extension through the existing [persistence contract](../architecture/capability-model.md#application-persistence-boundary), finite snapshot/admission, foundation, resolver/lock, binding and compiled lifecycle owners. Own the Checkout/durable receipt/entitlement guide example, including same-target site persistence.
4. **Intermediate / compatibility:** Never grant entitlement from a redirect or unverified browser claim. Verify raw webhook signature before state; durably record/queue with recoverable intent before acknowledgement; make processing replay/out-of-order safe. Minimal guest subject/access proof must be approved. Keep staging/live secrets and webhook configurations separate; never store/log/track card data. Site persistence uses exact finite supported dependency-only tuples, preserving public content, origin profile, defaults and optionals; inference, locks and APP_DB bindings must agree. Do not widen portfolio support, offer standalone public-profile persistence or invent a selectable consumer/certification scaffold.
5. **Tests / recovery:** Duplicate logical/event receipt, out-of-order/late events, forged/stale signature, concurrent checkout/replay, provider timeout, crash across receipt/enqueue/projection, failed/refunded payment, wrong subject and safe reconciliation. L/G plus no-money local stubs; real test-mode certification later. Exercise actual site generation and compiled addition/removal/re-addition, content/default preservation, coherent dependency inference/locks/bindings, same-target APP_DB and wrong-target refusal, export/restore, surviving references and retained-prefix recovery under the existing persistence contract. Environment acceptance: Use a general manual nonproduction sandbox shared by development/staging only without interference; ordinary CI uses controlled transport and authorized live-provider CI an isolated sandbox. Keep nonproduction credentials/customer/product/price IDs separate from live, endpoint signing secrets specific (including local forwarder), and Checkout return on the exact selected origin. Credential/event/account/target or raw signature mismatch produces no persisted receipt or entitlement. Use narrow signed ingress, never live charges/test customers; preserve one-time mode’s actual dependencies without requiring identity.
...
7. **Subjects / obligations:** Pending Stripe and any changed dependencies. The site extension needs separate exact persistence/foundation/deployment and any other materially changed shared-subject acceptance under the predecessor gate above; P5F-C0/CP/C supply no payment prerequisite. P8C-C later certifies one-time on site/app without identity and preserves independent method selection. Preserve P5C's exact app evidence; no certificate transfers to widened support.

**Execution checkpoints** — use section 3's per-increment gate with this card's ownership and claim boundary.

- [ ] **Admission / evidence gap:** Resolve the real site consumer and executable extension/certification sequence at preparation; obtain separate prerequisite acceptance before dependent site-payment implementation.
- [ ] **RED:** Expose untrusted checkout success, forged receipts, duplicate/out-of-order events and lost work between receipt, queue and entitlement projection; exercise the admitted site tuple and persistence lifecycle/refusal boundaries.
...
3. **Owners:** Billing projections and subject port, identity/account adapter, Stripe mapping and conditional support-console billing inspection/reconciliation. Own the subscription/Portal/billing-consumer reconciliation guide example.
...
5. **Tests / recovery:** Unowned portal, duplicate subscription events, older event overwriting newer projection, cancelled/unpaid/past-due states, period transitions, provider failure, account deletion conflicts, lost job and bounded audited reconciliation. Test both login and authorization, not redirects alone. Environment acceptance: Bind actual billing subject, subscription/Portal identity and browser return to the selected origin/environment. Exercise cancellation, failed payment, account-cleanup hooks and conditional support within that target. Preserve declared policies and the narrow provider-neutral billing port; no production customer is a fixture.
...
3. **Owners:** Billing mode/resolver/lifecycle, migration/subject mapping, conditional support workflows and security-sensitive checkout/portal actions. Own the both-mode migration/assurance and payment recovery guide examples.
...
5. **Tests / recovery:** One-time→both with existing history and replayed events, ambiguous subject, rollback after schema expansion, active subscription on removal, both-mode projection races, selected 2FA and passkey authorization journeys, environment mismatch. L/G and independent operational recovery review. Environment acceptance: Bind exact sandbox/live account and endpoint identities across one-time-to-both and selected assurance paths. Duplicate/out-of-order events, refunds/cancellations, enqueue/reconciliation failure and cleanup stay in the selected target. Reject live inputs on nonproduction and sandbox inputs on production; any legacy test-mode exception requires an actual feature constraint.
...
3. **Owners:** Calendly webhook adapter, signature/body validation, WebhookReceiptRepository and booking normalization, D1 migration, operator replay and builder selection/removal. Own the direct signed booking receipt/normalization/cancellation guide example.
...
5. **Tests / recovery:** Tampered/oversized payload, signature replay, duplicate/different event identity, cancellation before creation, reschedule ordering, transaction failure, response lost after commit, interrupted processing/replay, wrong environment and private diagnostics. L/G plus source-removal checks for retained data and active webhook configuration. Environment acceptance: Separate nonproduction account/event targets, subscriptions, signing secrets and callback URLs from production. Validate raw signature/replay window and expected account/subscription/environment before persistence through the exact signed method/path ingress. Local tests use controlled signed payloads; real development ingress needs an authorized reachable nonproduction endpoint. Never infer harmless scheduling or webhook proof from a browser embed/developer sandbox.
...
3. **Owners:** Booking-owned delivery intent/normalization, jobs transport/handler, operator runbook and optional composition/lifecycle. Own the direct/queued booking processing and replay recovery guide examples.
...
5. **Tests / recovery:** Commit/enqueue crash, duplicate queued cancellation, stale event after reconciliation, terminal replay, missing handler, changed payload schema and removal with outstanding work. G/L; distinguish source restoration, D1 recovery and provider subscription restoration. Environment acceptance: Use the receipt environment for optional queues and replay tools. Exercise receipt-commit/enqueue gaps, repeated cancellation, changed-handler failure and target-bound recovery; wrong target refuses before effects. An authorized staged signed route must work without opening other Access-protected routes; subscription/queue/credential cleanup is separately scoped.
...
3. **Owners:** Fleet evidence and capability/version/ownership owners; no new telemetry service or management framework. Own the representative fleet configuration and composed-flow examples.
...
5. **Tests / recovery:** Reproduce named inference/upgrade failures in sanitized fixtures where authorized; validate source identity and coverage selection. Retain replay seeds/paths only for materially combinatorial properties. No mutation means no fleet rollback claim. Environment acceptance: Include an environment-aware public project and stateful project with only real supported lifecycle histories. Do not resurrect retired generations or add a synthetic fleet service. Measure actual environment failures and retain the existing deferred performance scope.
...
3. **Owners:** The canonical failing inference/migration/adapter/package owner, all direct consumers and owning tests. Own the affected consumer guide and regression example.
...
5. **Tests / recovery:** Red reproduction, smallest GREEN, relevant complete suite, real compiled supported-edge fixtures, current/previous-major coverage when applicable, failure-prefix/recovery evidence and relevant deployed regression. Git recovery remains separately authorized. Environment acceptance: Repair only measured environment upgrade/maintenance weaknesses with causal tests. Do not invent support for absent consumers, revive retired tuples or introduce an environment framework.
...
3. **Owners:** Existing capability certifiers/recovery contracts and deployment policy; each domain keeps its own outcome owner. Own the selected environment recovery guide example.
...
5. **Tests / recovery:** Actual protected deployment and selected supported upgrade; stale backup, post-export write, partial restore, queue duplicates/poison, credential revocation, webhook resubscription and provider reconciliation as selected. Preserve failed attempts and prove baseline recovery before reusing environments. Environment acceptance: Bind each drill to exact target, source and subject; exercise forbidden cross-target operations and the actual data/provider recovery boundary. Separate controlled local, authorized deployed and recovery evidence; code reversal alone does not prove restored data.
...
**Deferred:** The [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred) excludes this alternate-adapter experiment and its contingent support/extraction decisions from active delivery and final acceptance. Imperative card text preserves requirements if reactivated. ADR-0004 isolation, ordinary in-memory/contract tests and ADR-0005 evidence-led package reassessment in P10-1/2 remain required.
...
3. **Owners:** One actual consuming-boundary port/adapter, architecture/package decision owners and contract suite. Own the bounded portability/support evidence and guide.
...
5. **Tests / recovery:** Run the shared behavior contract, explicitly compare transactions/conditional writes/queues/cancellation/storage semantics, measure bundle/maintenance tradeoffs and preserve counterexamples. Clean only the approved disposable proof; no provider cleanup is implied. Environment acceptance: Keep explicit target/binding semantics and real platform differences in the portability experiment. No second production adapter or generalized environment registry follows from this requirement.
...
1. **Outcome / exclusions:** At the end of all selected final fleet work, remove obsolete implementation-routing labels from durable architecture/governance/instruction surfaces, narrow scanner exemptions and present amended active-scope evidence/residual obligations, explicitly retaining the contact and portability deferrals. No rewrite of historical provenance, re-numbering or unsupported readiness claim.
2. **Predecessor:** All selected P10-2 repairs/renewals and P10-3 evidence accepted; every retained active phase/capability gate satisfied. P5F/contact certification and P10-4 decisions remain deferred under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred) and are not predecessors. Any unfinished retained obligation or pending executable subject still blocks applicable closure.
3. **Owners:** `scripts/check-semantic-naming.mjs` remains matcher/scan authority, enforcement map owns gate mapping, roadmap owns closure; update exact direct documentary consumers only. Own the final operator/service/composition guide and example inventory.
...
5. **Tests / recovery:** Scanner positive/negative behavior and tracked/nonignored inventory, link integrity, constitution/affected instructions, supported CLI docs, final registry admission and all-certified closure. Do not rerun unchanged expensive deployed evidence without a causal trigger. Review the exact documentation diff and preserve prior artifacts. Environment acceptance: Check target/setup commands, worked service/composed examples, future acceptance clauses and exact certification receipts for drift. Confirm safety and documentation already delivered by each capability; do not make this the first environment safety or guide gate.
6. **Commit / review / checkpoint:** One final documentation-hardening scope with R and the amended active-scope final packet. Explicit user approval names all retained outcomes, residual risks, the two deferrals and their limitations, support and any retained evidence-led extraction decision. Do not claim contact delivery, portability evidence or completion of the original entire program.
...
- [ ] **GREEN / acceptance evidence:** Run meaningful scanner behavior, link/constitution checks and final applicable admission/closure validation; present amended active-scope final acceptance with explicit residual obligations, contact/portability deferrals and the existing performance deferral.

## 7. Separate certification sequence and subject obligations

Rows below name separate certification tasks or explicitly named coordination checkpoints, not the final checklist of implementation. The three marked contact rows preserve future obligations under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred); they are excluded from active scheduling. All executable pending subjects and `all-certified` semantics remain unchanged. P7-CF coordinates individually accepted shared-subject tasks and a later composed-baseline checkpoint; it creates no extra composite certification task or registry identifier. All use C and R, exact implementation acceptance/ancestry, applicable L/G, private prerequisite planning, least-privilege protected staging when required and separate external approvals. Each proposes its own evidence/registry PR scope only if asked. Pending or failed predecessor evidence stops that task. Recovery plans cover source, dependency, deployment, data, provider and credentials separately as applicable.
...
| --- | --- | --- |
| P5D-C / P5D-3 | Certify email fresh generation and compiled addition/removal plus safe synthetic Resend API outcome and provider-confirmed delivery outcome under its approved runbook; no real recipients or inbox-guarantee claim | Throttle/auth rejection, ambiguous acceptance, duplicate/key-conflict, test delivery/failure and credential/domain disposition. Renew materially changed shared subjects individually. Environment evidence: Exact changed sender/recipient target in fresh/compiled lifecycle; controlled forbidden-recipient/mismatch refusal; separately authorized synthetic Resend delivery and target-bound secret/domain/cleanup disposition. |
| P5E-C / P5E-3 | Certify actual Queue delivery, retry, terminal failure and authorized replay in the chosen topology, plus L/G | Poison/duplicate/out-of-order, lost consumer, drain/replay/cleanup and isolated environment recovery. No exactly-once claim. Environment evidence: Local target/unsupported-operation negatives; separately authorized actual dispatch/retry/terminal/retention and later approved replay on exact queue/handler/schema; preserve local ack limits and no exactly-once claim. |
| **Deferred:** P5F-C0 / P5F-1 | Certify complete app-only durable contact on the already certified app/persistence baseline | Actual intake/lifecycle, stable post-commit event, D1 failure/duplicate retry, Turnstile abuse, status/privacy/export/delete and restore; no email/jobs claim. Environment evidence: App DB/Turnstile target negatives, dummy release rejection, authorized real staging challenge/intake and no-email acceptance, data cleanup/restore. |
| **Deferred:** P5F-CP / P5F-2 | Separate assertions/tasks for the accepted contact and persistence capabilities' newly supported public-profile tuples, plus exact changed shared subjects | Genuine public contact generation/compiled lifecycle, D1 binding, wrong-target refusal and export/restore/removal. Preserve P5C and P5F-C0 evidence; no impossible persistence-only public baseline and no restarted P5C. Environment evidence: Exact public contact+persistence pair; wrong target/binding/restore refusal, authorized staged public intake and target-bound export/restore/removal. |
| **Deferred:** P5F-C / P5F-4 | Certify final durable intake with separate no-email, direct-email and queued-email assertions | D1 failure, notification outage, lost enqueue, retry/reconciliation, Turnstile abuse, export/delete/restore; accepted submission survives notification failure. Shared certifiers do not replace contact assertions. Environment evidence: Separate no-email/direct/queued composed assertions with target consistency, notification outage/lost enqueue/duplicate reconciliation and accepted intake preserved. |
| P6-C / P6-3 | Certify protected CMS, both multilingual installation orders, staging import/parity, approved sole-authority cutover, export/ejection and recovery | Draft/media exposure, failed/partial import, stale parity, concurrent edits, CMS_DB/R2 restore and operator identities. Application customer identity remains separate. Environment evidence: Access-protected drafts/known media, both multilingual orders, paired DB/R2 import/parity/export/restore, wrong-target refusal, authorized cutover only and no production copy inference. |
| P7-CF / accepted complete P7-2, prerequisite shared-subject stage | Coordinate separate certification tasks for every materially changed shared prerequisite in actual declared dependency order, before dependent identity certification | Use the real complete authenticated baseline; pending co-installed identity subjects are not fictitious descriptor dependencies. Preserve prior exact-subject evidence. CF is a planning coordination reference, not a composite registry subject. Environment evidence: Separate exact shared subjects for complete environment-aware authenticated baseline in real declared dependency order; bind configuration/deployment/recovery and guides without certifying pending co-installed identity or creating a CF subject. |
| P7-CI / accepted P7-2 and certified exact declared shared prerequisites | Certify identity-core verified email/password, recovery, sessions, abuse/privacy and eject-only disposition on the complete first authenticated profile | Token replay/expiry, enumeration, stale session, failed migration, account lifecycle, secret/data cleanup. The deployed spike and other installed capabilities do not supply this certificate. Environment evidence: Auth URL/origin/cookie/secret/database/email-link separation; synthetic identity, cross-target token/session/recovery refusal and authorized staged identity/recovery/cleanup. |
| P7-CP / accepted P7-2 and accepted CI outcome | Certify protected-area route and use-case authorization, direct-object access and session revocation | Missing/forged/stale privilege, unsafe removal and server-side bypass cases. Keep its separate subject assertion even when scenarios overlap CI. Environment evidence: Access satisfied while missing/forged/stale app privilege and direct-object/API access fail; actual app authorization/revocation is observed independently of edge denial. |
| P7-CG / accepted P7-2 and accepted CI outcome | Certify actual Google sign-in/link/unlink and fallback | Callback/ownership mismatch, collision, provider outage, last-method recovery, token revocation and account cleanup. Environment evidence: Actual authorized staged Google sign-in/link/unlink; wrong project/client/redirect, Access expiry and safe last-method fallback; environment-bound token/account cleanup. |
| P7-CA / accepted P7-2 and certified declared core/persistence dependencies | Certify real owned profile CRUD/export/delete and hooks | Cross-user access, partial delete, retained legal/security data, export access, migration and restore/readback. Protected-area is not an invented descriptor prerequisite. Environment evidence: Same-target owned CRUD/export/delete/hooks/restore; wrong-target refusal, partial cleanup/retry and isolation from production sentinels. |
| P7-CS / accepted P7-2 and certified declared core/protected-area/account-profile dependencies | Certify bounded support operations, audit and confirmations, conditional contact only when installed | Non-admin action, role change, failed audit, disruptive-action confirmation, PII exposure and reconciliation retry. Environment evidence: Distinct staging admins and application roles, exact privileged confirmation/audit/conditional views; wrong target/non-admin/audit failure; no Access privilege inheritance. |
| P7-CF / final checkpoint after all shared and CI/CP/CG/CA/CS outcomes | Close the complete fresh authenticated baseline with all exact required subjects separately certified | Preserve site/client work and independent optionals; review composed security/privacy/runtime/deployment/recovery evidence. Reuse still-applicable causal outcomes; no duplicate certification or composite identifier. Migration and downstream consumption remain blocked until this checkpoint. Environment evidence: All exact shared and identity outcomes separately accepted; composed auth/DB/email/protected/account/support scenario, optionals preserved and reviewed recovery/docs. No composite certificate or duplicate rerun of unchanged causal evidence. |
| P7-CL / P7-3 | Renew every materially changed capability lifecycle subject for the exact incoming migration and applicable add/remove/ejection behavior | Compiled transition, data/source failure prefixes, preserved optional content, drift and unsafe-removal refusal, actual export/restore and required staging evidence before explicit P7 closure. Environment evidence: Exact supported new-generation transition/add/remove/ejection, refusal before wrong-target writes, preserved user work and actual target-bound restore/required staging evidence. |
| P8A-C / P8A-1 | Certify independently installed/added TOTP, challenge and single-use recovery with the approved method matrix | Lost factor, concurrent recovery use, replay/brute force, removal/downgrade and privileged recovery; renew affected core subject. Environment evidence: Environment-isolated factor/recovery/crypto material, lost factor/replay/concurrent recovery/removal and selected method assurance; stage reset cannot affect production. |
| P8B-C / P8B-1 | Certify passkey registration/use/revocation/recovery and agreed browser/device boundary | Origin/RP/challenge mismatch, replay, lost device, last-method removal; virtual authenticator evidence does not imply every physical-device flow. Environment evidence: Local virtual RP/origin/challenge and cross-target negatives, Access challenge expiry; authorized physical-device scope only where claimed; target revocation/recovery. |
| P8C-C / P8C-3 plus separately accepted payment-owned site persistence/shared subjects | Certify one-time, subscriptions and both; site/app one-time, authenticated subscriptions, one-time→both, selected assurance and conditional support journeys | Test-mode Checkout/Portal, duplicates/out-of-order, failed/refunded/cancelled states, reconciliation, safe removal, webhook/credential cleanup; no live-money or universal financial-compliance claim. Environment evidence: Controlled account/credential/event/endpoint mismatch negatives; separately authorized sandbox Checkout/Portal/signed ingress/entitlement, all modes and selected assurance, same-target failure/reconciliation/cleanup; no live-money claim. |
| P9-C / P9-2 | Separate protected-staging webhook subscription, synthetic booking/cancellation, signed normalized receipt, replay, direct and queued modes | Wrong signature, duplicate/reordered event, injected failures, receipt recovery, subscription/queue/credential cleanup. Never reuse front-end booking evidence as webhook proof. Environment evidence: Authorized narrow signed staging ingress and synthetic booking/cancellation, account/subscription/target/signature rejection, direct/queued receipt/replay/failure recovery and cleanup; embed proof remains separate. |
| P10 selected C renewals / each accepted affected repair or selected unchanged subject | Renew the specifically selected deployed/upgrade/recovery outcomes across approved representative fleet baselines | Bind environment/source/subject; distinguish old evidence, unchanged applicable evidence and actual new execution. Human accepts each subject outcome and resource disposition. Environment evidence: Renew only causally changed exact subjects using representative environment flows, targeted forbidden-action/recovery evidence, existing guides and separately authorized hosted proof; preserve all closure gates. |
...
| **D5 — contact** | Requirements preserved; delivery and contact certification deferred under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred). | On explicit reactivation, prepare concrete bounded abuse/privacy/retention settings and reuse accepted persistence support. |
...
| **D11 — fleet** | Retained fleet scope accepted; performance and the named P10-4 experiment stay deferred. | Actual authorized fleet/version/recovery targets; portability requires separate reactivation under the [contact and portability deferral](program-roadmap.md#contact-and-portability-work-deferred). |
