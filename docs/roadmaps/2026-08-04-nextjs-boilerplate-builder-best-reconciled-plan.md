...
- Current accepted `main@7509fc819ba8670040374c350762720848a47ef1` schedules later automated reference hardening without changing lifecycle behavior and is the direct predecessor for `apply-remove`; the implemented candidate is fingerprint-gated for exact `booking-calendly@0.1.0`, consumes the accepted `plan-remove` fingerprint, and stops at verified-final-diff as a separate approval.
...
- The currently implemented boundaries are read-only `plan-add`, fingerprint-gated `apply-add`, read-only `plan-remove`, and fingerprint-gated `apply-remove` for exact `booking-calendly@0.1.0`; application performs existing-repository transformation, migration append, and state-last persistence before stopping for verified-final-diff approval, while removal planning classifies ownership-aware dispositions and produces a redacted fingerprinted approval plan with no write. Every removal plan requires repository-wide surviving-reference review and exact-path reconciliation of any preserved/ejected capability surfaces before transformation approval; the current planner declares but does not automate those reviews. `apply-remove` is the implemented candidate for one exact ownership-aware transaction and retains verified-final-diff as a separate approval. Generic transformation, upgrades, automated recovery, later-add certification, P3B, and P3C runtime work remain planned.
