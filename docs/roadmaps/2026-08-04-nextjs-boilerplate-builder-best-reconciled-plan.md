...
- Current accepted `main@7509fc819ba8670040374c350762720848a47ef1` schedules later automated reference hardening without changing lifecycle behavior; the next direct removal-execution boundary is fingerprint-gated `apply-remove` for exact `booking-calendly@0.1.0`, which consumes the accepted `plan-remove` fingerprint and stops at verified-final-diff as a separate approval.
