...
## Retained analytics test resources

The [analytics certification workflow](../../.github/workflows/analytics-certification.yml) uses the separate `analytics-certification` environment, Worker, and concurrency group. Its additional Web Analytics permission and protected reviewer boundary keep it separate from `test-deploy`. These stable, capability-based identifiers are reusable across certification runs; provider dashboard display names should describe integration tests without a phase or date.
