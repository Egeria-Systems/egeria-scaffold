...
- 2026-08-09: Architecture review found that `site-routing` changed its owned about-route source contract to consume required accessibility input while remaining `0.1.0`. Advance it to `0.2.0` in `packages/builder-core/src/catalog/capability-catalog.ts`, exact resolution/generation/fixture contracts, and the generated site state so authoritative installed capability identity distinguishes the two source contracts. Preserve the six-capability boundary and current recipe version.
- 2026-08-09: Requirements review found that valid one-character skip, navigation, project, or action labels are not deterministically protected at 44 CSS pixels in both dimensions. Add causal assertions in `packages/builder-core/tests/render-skeleton.test.mjs`, then add `min-w-11` and centering to the affected anchors in the content-page and section-registry templates and regenerate both fixtures.
- 2026-08-09: Requirements review found that the new control-safe accessibility-label contract rejects C0 controls and DEL but not C1 controls. Add a `U+0085` regression in `packages/builder-core/tests/render-skeleton.test.mjs`, extend the shared generated content-string guard in `packages/builder-core/templates/common/apps/web/src/content/content-schema.ts` to reject `U+0080` through `U+009F`, and regenerate both fixture fingerprints. This closes the declared fail-closed control-safe contract without broadening accepted content.
- 2026-08-09: Accessibility review found that the skip link targets a `main` whose first focusable descendant is the repeated navigation, so activation does not bypass that block. Add a causal JSX-order assertion, move the navigation before and outside the focusable main target in `packages/builder-core/templates/common/apps/web/src/presentation/content-page.tsx`, preserve alignment/responsive wrapping, and regenerate both fixture fingerprints.
- 2026-08-09: Test-evidence review confirmed the 44-pixel classes are present but found that the test does not protect the `inline-flex` display required for minimum inline dimensions to apply. Assert `inline-flex` for the one-character skip, navigation, project, and action anchors in `packages/builder-core/tests/render-skeleton.test.mjs`; the implementation already satisfies this contract.
- 2026-08-09: Test-evidence review found that semantic design-token values are protected but their Tailwind `--color-*` mappings are not. Add an exact mapping assertion for all nine semantic colours in the focused global-style test so misspelled or removed mappings fail even when a production build still succeeds. No production CSS change is required.
