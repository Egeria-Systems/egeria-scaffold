...
- Bind any later external execution to these exact roles: GitHub repository administrator, workflow dispatcher with write access, required deployment reviewer with read access, Cloudflare account administrator, Calendly certification operator, and implemented-task reviewer. The Cloudflare token requires only `Workers Scripts Write` in the non-production account. The Worker preflight target is exactly `acme-portfolio-calendly`.
...
- Require every recorded evidence outcome to bind the capability, descriptor subject, evidence-producing revision, explicit `passed` outcome, and repository-present receipt metadata. A relabeled local document cannot satisfy deployed, provider, cleanup, or recovery evidence.
