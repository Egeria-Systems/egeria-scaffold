...
| `INV-ACCESSIBILITY-CLAIMS` | Automation alone never supports a conformance claim | actual in P0.1 docs; later automated | constitution contract and release checks | P0.1/P2 |
...
When a planned gate is implemented, update this table in the same focused change as the executable gate and its test. Record the exact command and result in implementation evidence. Passing a document contract proves only the documented rule and ownership; it does not prove a later runtime gate.
