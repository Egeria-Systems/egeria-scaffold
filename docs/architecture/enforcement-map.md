...
|---|---|---|---|---|
| `INV-PROFILE-MATERIALIZATION` | Resolved installed capabilities become authoritative; no live profile inheritance | planned; Task 2 resolver and manifest coverage exists, but inference and installed-state reconciliation are not implemented | `builder-core` resolver, manifest, and inference tests | P1 |
| `INV-CAPABILITY-METADATA` | Every capability declares normalized delivery, state, removal, security, ownership, inference, migration, and verification metadata | actual for the six executable descriptors and Task 3 pure ownership target/fingerprint materialization; remaining program catalog and filesystem inference enforcement planned | `builder-core` schema, catalog, and state-ownership contract tests | P1 |
