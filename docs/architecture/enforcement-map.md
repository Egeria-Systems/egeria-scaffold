...
|---|---|---|---|---|
| `INV-PROFILE-MATERIALIZATION` | Resolved installed capabilities become authoritative; no live profile inheritance | actual for deterministic resolution, installed-manifest projection, read-only state/capability evidence, and read-only diagnostics across desired, installed, and inferred capabilities; transformation-time reconciliation remains planned | `builder-core` resolver, manifest, repository-inference, and diagnostics tests | P1 |
| `INV-CAPABILITY-METADATA` | Every capability declares normalized delivery, state, removal, security, ownership, inference, migration, and verification metadata | actual for the six executable descriptors, pure ownership materialization, bounded probe evaluation, valid-state surface drift evidence, and content-safe diagnostic drift reporting; remaining program catalog enforcement planned | `builder-core` schema, catalog, state-ownership, repository-inference, and diagnostics contract tests | P1 |
