...
**Status:** Controlling package and API ownership through P1 Task 3
...
| `packages/builder-core` | `@egeria-systems/builder-core` | Private package | Runtime Zod contracts and checked schemas; exact six-capability catalog; `portfolio`/`site` recipes; deterministic resolver; installed-manifest projection; strict `.egeria` codecs; hybrid-ownership fingerprints and surface materialization | Canonical private owner of project/state schemas and cohesive builder internals; standards configuration consumer and future CLI consumer | `private: true`; root-only runtime exports plus package metadata; package-boundary source allowlist; no publication | Task 3 creates no `.egeria` files and performs no repository read; inference, diagnostics, generation, and repository mutation require later approved tasks; no separate project-schema package |
...
P1 Tasks 1 through 3 implement private contracts, six capability descriptors, two profile recipes, deterministic resolution, installed-manifest projection, strict state codecs, and pure ownership fingerprints/materialization only. They create no `.egeria` files, repository reader, inference result, generator, migration executor, provider, transport, application persistence, identity, payment, analytics runtime, generated client repository, or CLI behavior.
