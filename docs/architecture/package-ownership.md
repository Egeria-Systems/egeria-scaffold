...
| --- | --- | --- | --- | --- | --- | --- |
| `apps/cli` | `@egeria-systems/cli` | Private application | Empty ESM ownership shell; no `bin` | Future command input/output; no current consumer | `private: true`; no exports, dependencies, or lifecycle scripts | Commands and repository-changing behavior require later approved stages |
| `packages/builder-core` | `@egeria-systems/builder-core` | Private package | Empty ESM ownership shell | Future project/state schemas and cohesive builder internals; future CLI consumer | `private: true`; no exports, dependencies, or lifecycle scripts | P1 is the first executable schema and builder-kernel stage; no separate project-schema package |
| `packages/standards` | `@egeria-systems/standards` | Public ordinary dependency | Not materialized in the first P0.3 increment | Replaceable shared static standards with concrete consumers | A later P0.3 increment must add exact exports, a file allowlist, contract tests, versioning, and public-release defaults | Only justified, consumed standards APIs may be added |
...
- The CLI may depend on builder-core only after an approved executable builder stage creates a concrete need.
...
P0.3 private shells begin at `0.0.0`. The public standards and observability packages receive explicit Changesets release intent and package-content safeguards only in their approved P0.3 increments.
