...
**Runtime status:** Private builder-core validates ten current capability descriptors and three generation recipes, including exact `app-foundation@0.1.0` and `app@0.2.0`. App project/state parsing, rendering, and state-last new-directory generation are implemented under the [app boundary](#accepted-app-architecture-boundary), which owns runtime scope, verification, and CLI availability and remaining lifecycle gates. Six current local subjects are certified and four external subjects remain pending; earlier receipts remain historical evidence only for their exact prior descriptor digests. Exact incoming app planning and execution follow the boundary below; the two representative immutable app fixtures use the [retained-fixture verification boundary](#representative-immutable-app-fixtures), while certification execution remains later work. Every other program capability and profile remains documentation-only.
...
The generation-executable recipes are `portfolio@0.11.0`, `site@0.12.0`, and `app@0.2.0`, selecting standards `0.5.0` and Vitest `5.0.0` under [ADR-0015](../adr/0015-vitest-five-generation.md). Site and app retain the reviewed Next.js security patch and exact `site-routing@0.4.0`; portfolio retains its existing Next declaration and security override. Explicit retained snapshots preserve earlier generated bytes, descriptors, locks, diagnostics, and supported lifecycle behavior independently of these defaults. The routing descriptor declares its generated nested error boundary's dependency on the already-installed exact `observability@0.3.0`; every established capability dependency remains unchanged, including exact `deployment-cloudflare@0.3.0`; the independently versioned standards capability advances only for new generation. An initial-scaffolding request may explicitly add `booking-calendly`; dependency-first resolution adds its existing `section-composition` dependency, and the generated installed manifest records the resulting selection. Exact `app@0.2.0 = app-foundation@0.1.0 + site-routing@0.4.0` is accepted by the shared profile/project/state contracts and builder-core generation; the [app boundary](#accepted-app-architecture-boundary) owns CLI availability and the remaining lifecycle gates.

Multilingual and analytics remain independent optional capabilities during initial portfolio/site/app generation; exact later addition/removal supports all three profiles under the existing lifecycle preconditions; CLI creation supports portfolio/site/app. There is no composite profile or capability and no client-specific default recipe. Exact `multilingual@0.1.0` and exact `analytics@0.1.0` remain separate executable subjects. `site-routing` is a default capability for site and app, while each optional capability retains its own state, dependencies, migrations, verification, certification, and removal policy.

## Initial catalog

The current catalog contains the nine established descriptors plus exact `app-foundation@0.1.0`. The established descriptors support portfolio, site, and app where declared; `site-routing@0.4.0` supports site and app, while `app-foundation` supports only app. Current recipe-catalog, project/state, rendering, and new-directory generation entries are `portfolio@0.11.0`, `site@0.12.0`, and `app@0.2.0`. App CLI availability and the remaining lifecycle gates follow the [app boundary below](#accepted-app-architecture-boundary). The other rows remain program visibility only and have no runtime descriptor, resolver availability, generated surface, state, or provider effect yet.
...
Read-only planning and fingerprint-gated execution retain exactly `portfolio@0.10.0 -> app@0.1.0` and `site@0.11.0 -> app@0.1.0`, and add same-generation edges `portfolio@0.11.0 -> app@0.2.0` and `site@0.12.0 -> app@0.2.0` under ADR-0015. New portfolio visual inputs require the separate exact-manifest approval below. No mixed-generation transition or automatic standards `0.4.0` to `0.5.0` upgrade is introduced. Planning must classify every relevant path as preserve, create, replace, migrate structurally, or refuse; perform no UI or content deletion; preserve unrelated application-owned bytes; bind all influencing inputs; and refuse uncertainty before mutation. App-foundation has no removal executor or back-transition.

App-foundation and the nine existing descriptors extended to support app are ten exact certification subjects. Six current local subjects are certified and four external subjects remain pending under [certification coverage](#certification-coverage). The six accepted records use the coordinated certification plan; unchanged pending records retain their plans. Historical evidence remains valid only for its exact prior descriptor digest. Lifecycle implementation alone does not certify a subject or authorize provider action.
...
Builder-core admits exact app project/state contracts and state-last new-directory generation. CLI `create` accepts `portfolio`, `site`, and `app` through the shared profile identifier. The existing `plan-profile-transition` and `apply-profile-transition` commands accept only `site` or `app` targets and delegate the exact declared edges to builder-core: historical `portfolio@0.10.0 -> site@0.10.0` and the four incoming app edges above. They accept no caller-supplied source profile or new capability/settings input. The CLI preserves the existing content-safe JSON, stable exit, approval, and recovery envelopes; policy, action construction, verification, and persistence remain in builder-core. Public capability-addition and capability-removal planners support exact app projects for `booking-calendly@0.1.0`, `multilingual@0.1.0`, and `analytics@0.1.0` whenever the ordinary dependency, ownership, removal-policy, reference, Git, and approval preconditions pass. The existing executors recompute the plan, preserve unrelated capabilities/settings and compatible application-owned bytes, verify and re-infer, append the migration before persisting state, and finish with exact-byte verification and the verified-final-diff approval stop. No app-specific algorithm or migration family is added. The unchanged conservative reference guard refuses multilingual removal while the surviving generated analytics browser specification references the removable locale module; fresh-render compatibility does not bypass that precondition. App capability upgrades remain unsupported and have no app verification vector. Incoming app execution follows the exact transaction boundary below; the two representative immutable app fixtures use the [retained-fixture verification boundary](#representative-immutable-app-fixtures), while certification execution remains later work. Transition visual promotion requires the separate exact-manifest human approval described below. The [program roadmap](../roadmaps/program-roadmap.md#p4--app-foundation) owns their sequencing and approval gates.
...
### Representative immutable app fixtures

`fixtures/generated/app` retains default `app@0.2.0`. `fixtures/generated/app-all-optional-integrations` selects the existing Calendly, multilingual, and analytics capabilities together with the existing synthetic analytics settings. These are the only retained app fixtures; the eight generation subsets and all eight optional subsets per retained and current incoming edge remain at their existing builder-core owners.

The compiled CLI generates each fixture into an absent, identity-owned temporary destination using production verification. Exact rendered files, descriptor ownership, manifests, approved recipe lockfile, generated instructions, workflow ordering, deployment configuration, and final project/state/inference agreement are inspected before promotion. The Vitest 5 increment regenerates all seven current fixture roots through this owner; explicit retained snapshots and historical-output comparisons protect the earlier Vitest 4 generation. [`generatedFixtureContracts`](../../scripts/verify-generated-skeletons.mjs) owns the exact fixed inventory and identities; its owning tests bind them to the current descriptors and generated contracts.

New generation declares exact `vitest@5.0.0`, uses the corresponding new lock, and removes only the obsolete Vitest 4 override. The public standards package remains `0.1.0`. Historical fresh app `0.1.0` still declares `4.1.11`; historical portfolio/site generation and incoming app transitions retain `4.1.10` with the approved override to `4.1.11`. The retained standards probe recognizes exactly those two historical declarations, while standards `0.5.0` requires `5.0.0`; recorded fingerprints remain exact. Optional operations preserve each installed declaration and lock. Historical descriptors, recipes, locks, and approved visual input records remain unchanged.

Committed fixtures are immutable inputs. The fixed-root verifier installs, builds, and runs browsers only in identity-bounded temporary copies. After OpenNext build and before browsers, it runs `pnpm --dir apps/web run --if-present test:integration:cloudflare`. A present script must succeed or return `WORKER_INTEGRATION_FAILED`. Results identify executed fixtures separately from verified absent-script skips; the common check vector does not imply Worker execution for non-app fixtures. App inspection reuses the source-import and installed-Effect/client-bundle checks owned by the app transition visual verifier, with the exact Effect pin and native-build denial preserved. Non-app manifests and dependency graphs remain Effect-free.

Default app reuses the approved production-site visual inputs. The all-optionals fixture reuses the owning multilingual/analytics presentation evidence without creating a new visual matrix. The [visual regression boundary](#executable-visual-regression-boundary) retains its pinned environment, baseline approval, thresholds, failure artifacts, and cleanup rules. Separate default app-transition comparisons remain required under their existing eligibility. Exact executed commands and source/build identities belong to the ignored fixture review packet; these checks do not certify any capability or establish deployment, provider confirmation, translation quality, visual quality, production safety, human accessibility, or WCAG conformance.

### Read-only incoming app transition boundary

The pure app-profile-transition policy consumes explicit source/target bytes, canonical ownership descriptors, validators, fingerprints, and reviewed visual evidence. The existing planner shell owns bounded repository and Git reads. All eight optional subsets per retained and current incoming edge retain exact selections and settings, preserve compatible application-owned bytes and unrelated valid package members, and classify the complete inventory without delete actions. Unknown identities, ejection, managed drift, invalid controls/content, collisions, changed Git identity, and unproved visual inputs refuse before any write or verifier execution.

Incoming app edges merge only `/dependencies/effect` and `/scripts/test:integration:cloudflare`; portfolio additionally changes `/dependencies/next` and `/devDependencies/eslint-config-next`. An unrepresented dependency graph cannot use the fixed lockfile and therefore refuses. Builder-kernel replaces the root lockfile with exact `web-recipe-app-0.1.0` or `web-recipe-app-0.2.0` bytes selected for the matching generation and replaces the recognized workspace policy to retain the explicit native-build denial. New `.egeria` persistence remains outside the transformation batch. Private plan fingerprints bind raw and parsed controls, manifests, descriptors, recipes, ownership, settings, Git identity, every preserved path and action byte, and reviewed first-viewport evidence. Public action output contains sorted relative paths and stable reasons only.
...
Historical `standards@0.4.0` introduced deterministic visual regression, which new-generation `standards@0.5.0` retains without changing the exact public `@egeria-systems/standards@0.1.0` package pin. It owns the managed visual configuration and merge-managed `test:visual` script; the generated project owns the application-owned specification and profile baselines. The configuration consumes already prepared OpenNext/workerd preview output, one Chromium worker, fixed `en-CA` locale and Toronto timezone, light colour scheme, reduced motion, hidden caret, disabled animations, CSS-pixel scaling, and exact zero-pixel tolerance.

The representative matrix contains four screenshots only: portfolio at 1440 by 900 and 320 by 800 CSS pixels, and site at those same two viewports. The optional Calendly portfolio reuses the portfolio baseline bytes while retaining its separate behavioral browser test. The default app fixture reuses the site baseline bytes. The multilingual fixtures, including the all-optionals app, reuse their capability-owned presentation and behavioral evidence without a retained visual comparison. No browser, locale, route, colour-scheme, or full-page combinatorial matrix is implied.
...
The prior `multilingual@0.1.0` descriptor subject was certified from separately planned and accepted `fresh-scaffold` and `existing-repository-lifecycle` evidence at revision `96b587a254cf6fc859867d6fc66c7e0c900c4cfd`; the current app-supporting subject is separately certified at `fe16ae41153f89242cdf48c0f899c3720f511557` under [current certification coverage](#certification-coverage). Implementation evidence, ordinary unit/component/browser/build results, and registry admission alone do not certify multilingual behavior. The accepted historical certificate does not establish linguistic quality, visual quality, human accessibility, deployed behavior, production readiness, privacy completeness, or WCAG conformance. Analytics implementation and the combined real-client journey required separate successor acceptance.
...
The current registry records accepted local certification for exact `app-foundation@0.1.0`, `standards@0.5.0`, `content-files@0.4.0`, `section-composition@0.3.0`, `site-routing@0.4.0` and `multilingual@0.1.0`, each bound to its current descriptor digest and evidence revision `fe16ae41153f89242cdf48c0f899c3720f511557`. Six separately accepted content-safe receipts are linked from the [registry](../../certifications/capabilities.json). Fresh app, applicable compiled lifecycle, whole-Worker, Effect and strict visual evidence passed at that revision, with actual user review of the subject-specific documentation and recovery limits. Exact analytics, booking-calendly, deployment-cloudflare and observability remain pending until their external outcomes and reviews complete. This partial certification does not close the [app-foundation phase](../roadmaps/program-roadmap.md#p4--app-foundation), establish production readiness or authorize a successor.
