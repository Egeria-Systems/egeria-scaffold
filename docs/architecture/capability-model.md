...
**Status:** Controlling capability vocabulary through the current Calendly initial-scaffolding and restricted error-diagnostics implementation
...
Repository removal, deployment-observability configuration, credential revocation or rotation, provider-resource disposition, retained-data disposition, and source/provider/credential recovery are separate reviewed requirements. The current subject under [`certifications/capabilities.json`](../../certifications/capabilities.json) is exact `observability@0.3.0`, behavior-contract digest `sha256:24a3cb3361cd8f72a12a1926b512e087adb31ad120a62b70e06a68d9dcf90c99`, ordinary `pending`, empty evidence, and linked to the active private local restricted-error-diagnostics certification plan. Its local `fresh-scaffold` outcome at `bec51540c9ff4b68af790e9413cd9d1102d54396` is reviewed and accepted; no deployed-application or cleanup-recovery evidence has been produced. The prior certified `observability@0.2.0` subject at `ee1e1df10fa2be2f09333efecd86de7f7a131d49` retains only historical evidence from the private protected-staging receipt; that exact prior subject cannot certify `0.3.0`. Current local/static/generated evidence establishes no deployed/provider receipt, retention, cleanup/recovery, durable delivery, production readiness, privacy completeness, visual quality, human accessibility, or WCAG-conformance proof.
...
Clean-checkout admission is actual and rejects absent, unknown, stale-version, stale-digest, or unauthorized backfill records without reading ignored private artifacts. The exact standards subject is certified from its task-linked local receipt, exact `observability@0.3.0` is ordinary `pending` with empty evidence under its active private local certification plan with reviewed local `fresh-scaffold` evidence at `bec51540c9ff4b68af790e9413cd9d1102d54396` outside the tracked registry, four unchanged subjects remain `backfill-pending`, and `booking-calendly` remains certified. Before a certification workflow updates the tracked registry, the separately invoked local private-artifact gate requires every referenced plan and evidence receipt and binds each passed entry and receipt to its capability, descriptor subject, evidence-producing ancestor revision, and declared outcome. Receipts must also be complete, contain no unresolved prompt fields, and record affirmative overall and per-outcome review. Descriptor admission passes; `legacy-backfill-exempt` transition closure rejects pending observability, and `all-certified` closure rejects pending observability plus the four frozen backfills.

The actual fresh-scaffold runner creates a new `portfolio` project through the compiled CLI, re-infers it, requires healthy diagnostics and an empty exact diff, and reuses the fixed generated-project install/build/browser verifier against that output. For standards, its compiled runner plus explicit test counts, retained-fixture verification, and static CI-contract review supply the subject-bound local receipt. The earlier observability workflow and reviewed receipt supplied only historical `observability@0.2.0` fresh-scaffold and deployed-application evidence; cleanup was not executed or claimed. The active private local `observability@0.3.0` certification plan has accepted exact-subject local `fresh-scaffold` evidence at `bec51540c9ff4b68af790e9413cd9d1102d54396`; deployed-application and cleanup-recovery evidence have not run. A separately authorized exact-revision workflow and content-safe human receipt record the passed protected-staging application, provider-confirmed booking, cancellation, and cleanup outcomes that certify `booking-calendly@0.1.0`. These bounded journeys do not prove ongoing provider availability, production readiness, or WCAG conformance.
