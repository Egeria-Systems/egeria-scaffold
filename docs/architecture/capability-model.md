...
**Runtime status:** The seven `portfolio`/`site` descriptors and two recipes named below are executable in private builder-core. Exact `standards@0.4.0` is certified from accepted local `fresh-scaffold` evidence at revision `416e2c2441978ac86f3a17dee96a694141033e20` after the visual-regression material change. `booking-calendly` is an explicit optional initial-scaffolding selection with a certified subject-bound record, actual fresh-scaffold evidence, and passed protected-staging, provider-confirmed, and cleanup evidence; every other program capability and profile remains documentation-only.
...
The executable `portfolio` and `site` recipes are `0.10.0`. Their capability identifiers and dependency graph remain unchanged; the recipe advance records exact `standards@0.4.0` visual regression while retaining exact `deployment-cloudflare@0.3.0`, exact `observability@0.3.0`, and the unchanged public package pins. An initial-scaffolding request may explicitly add `booking-calendly`; dependency-first resolution adds its existing `section-composition` dependency, and the generated installed manifest records the resulting selection.
...
Task 8B is complete for exact `standards@0.4.0` from the accepted local `fresh-scaffold` outcome at evidence revision `416e2c2441978ac86f3a17dee96a694141033e20`. Historical `standards@0.3.0` evidence cannot certify the new subject. Screenshot equality is narrow evidence for selected pixels in the pinned environment; it does not establish visual quality, design quality, human accessibility, assistive-technology behavior, deployed behavior, production readiness, or WCAG conformance. Performance budgets are deferred by the unnumbered P2 closure amendment and no performance claim is made; neither controlled-laboratory nor field or real-user performance evidence is established. The documentary deferral does not alter `standards@0.4.0`, behavior-contract digest `sha256:8733f70cdc64134232912c691c6922b27defb8cb7c2871faa334cfad2b394643`, recipe `0.10.0`, or accepted certification evidence, so no certification sibling is required. No analytics, provider resource, deployment, or real-client generation is part of this boundary.
...
Clean-checkout admission is actual and rejects absent, unknown, stale-version, stale-digest, or unauthorized backfill records without reading ignored private artifacts. Exact `standards@0.4.0` is certified from its accepted task-linked `fresh-scaffold` receipt at evidence revision `416e2c2441978ac86f3a17dee96a694141033e20`; `observability@0.3.0`, `deployment-cloudflare@0.3.0`, and `booking-calendly@0.1.0` remain certified from their exact task-linked receipts, and three unchanged subjects remain `backfill-pending`. Before a certification workflow updates the tracked registry, the separately invoked local private-artifact gate requires every referenced plan and evidence receipt and binds each passed entry and receipt to its capability, descriptor subject, evidence-producing ancestor revision, and declared outcome. Descriptor admission passes, `legacy-backfill-exempt` closure passes, and `all-certified` closure rejects only the three remaining `backfill-pending` subjects.

The Task 8B successor runner used the compiled CLI for fresh scaffolding of a `portfolio` project, re-inferred it, required healthy diagnostics and an empty exact diff, and reused the existing fixed generated-project verifier, including deterministic visual regression. Its accepted receipt certifies only exact `standards@0.4.0` at evidence revision `416e2c2441978ac86f3a17dee96a694141033e20`; historical `standards@0.3.0` evidence remains limited to that prior subject. The accepted `observability@0.3.0` and `booking-calendly@0.1.0` evidence remains bound to those exact subjects. These bounded journeys do not prove ongoing provider availability, visual quality, production readiness, or WCAG conformance.
...
The implemented existing-repository boundary is limited to read-only `plan-add`, fingerprint-gated `apply-add`, read-only `plan-remove`, and fingerprint-gated `apply-remove` for exact `booking-calendly@0.1.0`, plus accepted read-only `plan-upgrade` and accepted exact `apply-upgrade` for the single `standards@0.3.0` to `standards@0.4.0` edge, accepted read-only `plan-profile-transition`, and this candidate's exact fingerprint-gated `apply-profile-transition` for exact `portfolio@0.10.0` to `site@0.10.0`. Application requires the same clean attached linked worktree and exact private plan input, transforms once, verifies an isolated copy without VCS metadata, requires pending inference and the exact transformed bytes, appends one successful migration record containing only completed checks, persists state last with the same conservative receipt, requires final state/inference and the exact dirty paths, then rereads exact migration/source/control bytes before stopping for verified-final-diff approval. Removal planning verifies the installed recipe and exact managed/shared bytes; classifies unchanged capability-owned application files for deletion and modified or already-ejected application files for preservation/ejection; and returns a redacted fingerprinted approval plan with no write. Its fingerprint-bound review requirements require repository-wide review for surviving references to removed surfaces and exact-path reconciliation for any preserved/ejected capability file before transformation approval. The current planner declares those requirements but does not scan arbitrary repository source. Exact `apply-remove` is accepted-main integrated at `main@7f59e8b093edb7be617cd2a30bfb4ebaa6a8ab6e`; post-merge Repository quality run `32620215344` passed every applicable job. It consumes the accepted `plan-remove` fingerprint for one exact ownership-aware Calendly transaction before a separate final-diff gate. Merged pull request 48 makes the standards executor accepted-main integrated; merged pull request 49 makes read-only portfolio-to-site planning accepted-main integrated. Acceptance of exact `apply-profile-transition` remains pending this candidate's integration. Generic lifecycle transformation, another upgrade or profile-transition edge, automated recovery, provider/data/credential cleanup, P3B, P3C runtime work, and later-add certification remain planned. The boundary adds no webhook, new profile, provider action, deployment, publication, or certification.
