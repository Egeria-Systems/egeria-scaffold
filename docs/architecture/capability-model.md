...
The executable `portfolio` and `site` recipes are `0.9.0`. Their capability identifiers remain unchanged; the recipe advance records exact pending `deployment-cloudflare@0.3.0` and its managed manual workflow while retaining exact `observability@0.3.0`, its safe/diagnostic split, five generated error UI/content surfaces, and the default `standards@0.3.0` testing contract. An initial-scaffolding request may explicitly add `booking-calendly`; dependency-first resolution adds its existing `section-composition` dependency, and the generated installed manifest records the resulting selection.
