...
**Runtime status:** The seven `portfolio`/`site` descriptors and two recipes named below are executable in private builder-core. Exact `standards@0.4.0` is pending its separately planned fresh-scaffold certification after the visual-regression material change. `booking-calendly` is an explicit optional initial-scaffolding selection with a certified subject-bound record, actual fresh-scaffold evidence, and passed protected-staging, provider-confirmed, and cleanup evidence; every other program capability and profile remains documentation-only.
...
Baselines are application-owned binary PNGs committed beside their specification and created or changed only in the exact digest-pinned Linux/amd64 Playwright environment. `--update-snapshots` is permitted only after a causal source change, followed by human review of expected, actual, and diff images and a second comparison without update mode. CI never updates baselines. The existing failure-only Playwright report and test-result artifact contains available expected, actual, and diff images and is retained for seven days, subject to any lower repository or organization maximum. Removing the capability removes managed configuration and the script property through reviewed capability removal; application-owned specifications and baselines require explicit human disposition rather than automatic deletion.

Task 8B remains pending as a separate fresh-scaffold certification for the exact `standards@0.4.0` subject. Historical `standards@0.3.0` evidence cannot certify the new subject. Screenshot equality is narrow evidence for selected pixels in the pinned environment; it does not establish visual quality, design quality, human accessibility, assistive-technology behavior, deployed behavior, production readiness, or WCAG conformance. Performance budgets are the immediately following separate increment and remain unimplemented. No analytics, provider resource, deployment, or real-client generation is part of this boundary.
...
Clean-checkout admission is actual and rejects absent, unknown, stale-version, stale-digest, or unauthorized backfill records without reading ignored private artifacts. Exact `standards@0.4.0` is pending separate fresh-scaffold certification with no evidence; `observability@0.3.0`, `deployment-cloudflare@0.3.0`, and `booking-calendly@0.1.0` remain certified from their exact task-linked receipts, and three unchanged subjects remain `backfill-pending`. Before a certification workflow updates the tracked registry, the separately invoked local private-artifact gate requires every referenced plan and evidence receipt and binds each passed entry and receipt to its capability, descriptor subject, evidence-producing ancestor revision, and declared outcome. Descriptor admission passes; both `legacy-backfill-exempt` and `all-certified` closure reject the pending standards subject.

The historical standards fresh-scaffold runner creates a new `portfolio` project through the compiled CLI, re-infers it, requires healthy diagnostics and an empty exact diff, and reuses the fixed generated-project verifier. Its existing receipt certifies only `standards@0.3.0`; Task 8B must separately plan and execute the successor `standards@0.4.0` fresh-scaffold evidence. The accepted `observability@0.3.0` and `booking-calendly@0.1.0` evidence remains bound to those exact subjects. These bounded journeys do not prove ongoing provider availability, visual quality, production readiness, or WCAG conformance.
