...
The descriptor owns the locale routing, validation, presentation, generated specifications, and both application-owned locale catalogs. Existing single-locale sources remain an inactive recovery baseline while the capability is installed. Shared builder-kernel, content, standards, observability, site-routing, and Calendly files keep their existing owners; exact composition replaces their rendered bytes and fingerprints without creating overlapping ownership. Addition supports Calendly before multilingual and multilingual before Calendly. Removal restores shared single-locale bytes, deletes unchanged managed multilingual surfaces, and requires reviewed preservation or ejection of modified application-owned locale catalogs. Source recovery, repository recovery, and any future provider or deployed recovery remain separate.

Multilingual certification is separate and remains pending until the task passes both `fresh-scaffold` and `existing-repository-lifecycle` evidence. Implementation, local unit/component/browser/build evidence, and registry admission do not certify multilingual behavior and do not establish linguistic quality, human accessibility, deployed behavior, production readiness, privacy completeness, or WCAG conformance. Analytics implementation and the combined real-client journey remain separate successors.
