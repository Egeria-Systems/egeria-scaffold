...
The generated app's `test:integration:cloudflare` script runs its Vitest integration configuration against the whole built Worker through `createTestHarness()`. Ordinary generated-project verification runs the conditional integration command after OpenNext build; generated quality and deployment workflows place the same command before browser or credential-bearing deployment steps. Exact app generation and optional-capability addition/removal require `worker-integration` immediately after `opennext-build`, with the owned script present and successfully completed; portfolio/site vectors remain unchanged. Generation validates the receipt against the rendered capability graph, and the lifecycle executors select the existing exact app receipt before migration/state construction. [State contracts](../../packages/builder-core/src/contracts/state.ts) own exact persisted and full verification vectors, including active app addition/removal receipts and staged app transition receipts. Generated test source is not execution evidence: actual command outcomes, bundle boundaries, cancellation behavior, browser results, and limitations belong to the implementation evidence and review packet. No local result implies deployment, ongoing provider availability, accessibility conformance, or production readiness.
