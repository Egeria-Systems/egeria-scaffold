...
The first implemented boundary is the read-only `plan-add` eligibility plan for `booking-calendly@0.1.0`; existing-repository transformation, migration, and state persistence remain planned. This is eligibility planning only: no existing-repository transformation and no later-add certification is implemented. It adds no webhook, new profile, provider action, or generated-project mutation.
