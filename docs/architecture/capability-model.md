...
Read-only planning supports exactly `portfolio@0.10.0 -> app@0.1.0` and `site@0.11.0 -> app@0.1.0`; their execution remains gated. Planning must classify every relevant path as preserve, create, replace, migrate structurally, or refuse; perform no UI or content deletion; preserve unrelated application-owned bytes; bind all influencing inputs; and refuse uncertainty before mutation. App-foundation has no removal executor or back-transition.
