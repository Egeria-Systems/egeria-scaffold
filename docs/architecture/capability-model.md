...
The approved P3B boundary retains multilingual and analytics as optional capabilities during initial scaffolding and addable later through the completed P3 lifecycle; there is no composite profile or capability and no client-specific default recipe. This candidate delivers only exact `multilingual@0.1.0` for executable `portfolio` and `site`; analytics remains a separate later capability. `site-routing` remains the default capability for `site`, while each optional capability retains its own state, dependencies, migrations, verification, certification, and removal policy.

## Initial catalog

The accepted base catalog contains exactly `standards`, `content-files`, `section-composition`, `deployment-cloudflare`, `observability`, `booking-calendly`, and `site-routing` for `portfolio` and `site`. The approved candidate adds exact `multilingual@0.1.0` as the eighth executable descriptor for those two profiles. The other rows remain program visibility only and have no runtime descriptor, resolver availability, generated surface, state, or provider effect yet.
