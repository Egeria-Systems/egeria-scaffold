...
| `multilingual` | `source-generated` | `repository-stateful` | `reviewed` | optional: portfolio, site | `content-files`, `section-composition`, `observability`; optional composition with `site-routing` and `booking-calendly`; app and authenticated-app remain future visibility only |
| `analytics` | `hybrid` | `repository-stateful`, `external-stateful` | `reviewed` | optional: portfolio, site | Public profile; each provider selected independently |
...
Analytics certification remains a pending separate successor. Its exact subject requires `cleanup-recovery`, `deployed-application`, `existing-repository-lifecycle`, `fresh-scaffold`, and `provider-confirmed` evidence. No provider provisioning or mutation, no deployment, no legal-compliance claim, client data, credentials, spend, analytics certification, or combined real-client action is authorized by implementation.
