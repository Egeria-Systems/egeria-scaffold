...
App-foundation and the nine existing descriptors extended to support app are ten current pending certification subjects under the coordinated plan. Historical evidence remains valid only for its exact prior descriptor digest and cannot certify these subjects. This generation increment changes no certification status and authorizes no certification execution or provider action.
...
Builder-core admits exact app project/state contracts and state-last new-directory generation. CLI `create --profile app` remains rejected by a local schema derived from the shared profile identifier until its separately approved CLI increment. Public capability-addition and capability-removal planners refuse app with their existing stable unsupported codes before rendering; the apply boundaries recompute those planners, so an approval fingerprint cannot bypass the staged refusal. App capability upgrades remain unsupported and have no app verification vector. Incoming app transitions, dedicated retained app fixtures, visual acceptance, and certification execution remain later work. The [program roadmap](../roadmaps/program-roadmap.md#p4--app-foundation) owns their sequencing and approval gates.

The generated app's `test:integration:cloudflare` script runs its Vitest integration configuration against the whole built Worker through `createTestHarness()`. Ordinary generated-project verification runs the conditional integration command after OpenNext build; generated quality and deployment workflows place the same command before browser or credential-bearing deployment steps. Only exact app generation with the owned script present and successfully completed may return `worker-integration` immediately after `opennext-build`; portfolio/site vectors remain unchanged. Generation validates that receipt against the rendered capability graph before state construction. [State contracts](../../packages/builder-core/src/contracts/state.ts) own exact persisted and full verification vectors, including the staged app addition/removal/transition receipts. Generated test source is not execution evidence: actual command outcomes, bundle boundaries, cancellation behavior, browser results, and limitations belong to the implementation evidence and review packet. No local result implies deployment, ongoing provider availability, accessibility conformance, or production readiness.
...
Exact `multilingual@0.1.0` is source-generated, repository-stateful, and reviewed. It is independently selectable for `portfolio`, `site`, and `app` during initial generation; exact fingerprint-gated later addition and removal remain available only for portfolio/site under the [staged app boundary](#accepted-app-architecture-boundary). It adds no package, provider, credential, platform resource, persistent data, external action, new profile, or generic lifecycle API. Default generation without multilingual remains byte-stable.
...
Exact `analytics@0.1.0` is hybrid, repository-stateful and external-stateful, with reviewed removal. It is independently selectable for exact current `portfolio`, `site`, and `app` initial generation; later addition/removal remains available only for portfolio/site under the [staged app boundary](#accepted-app-architecture-boundary). It is not a recipe default and has no dependency on `observability`. Its generated sources import no observability package or adapter and emit no observability event. The implementation adds no public package, provider credential, secret, provider API client, generic lifecycle executor, profile, persistent-data store, or deployed resource.
