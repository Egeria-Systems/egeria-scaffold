...
Exact fingerprint-gated compiled `apply-upgrade` is now a local reviewed-but-unaccepted Gate 3 candidate for only that edge. Its private operation-specific executor/writer recomputes planning and Git identity before the six-action write, transforms once, verifies and freshly infers before the exact migration append, persists state last with `lastSuccessfulVerification.kind: "capability-upgrade"`, reruns agreement, proves the exact eight-path diff, and stops for separate verified-final-diff approval. Planning approval, execution, verification, persistence, recovery, and verified-final-diff approval remain separate. Every failure after a committed write retains an inspectable prefix and never triggers automatic rollback. This local candidate is not accepted, merged, certified, deployed, published, production-ready, or finally approved; local checks do not prove browser/workerd or deployed behavior, provider behavior, visual quality, accessibility conformance, lifecycle or security certification, or production readiness. Generic lifecycle extraction still waits for accepted concrete add, removal, upgrade, and profile-transition implementations and remains internal unless later evidence independently satisfies package extraction. The finite single-edge matrix retains named examples; `fast-check` remains deferred until a later graph is materially combinatorial.
