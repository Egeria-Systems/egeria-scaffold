...
`fixtures/generated/app` retains default `app@0.1.0`. `fixtures/generated/app-all-optional-integrations` selects the existing Calendly, multilingual, and analytics capabilities together with the existing synthetic analytics settings. These are the only retained app fixtures; the eight generation subsets and sixteen incoming-transition cases remain at their existing builder-core owners.

The compiled CLI generates each fixture into an absent, identity-owned temporary destination using production verification. Exact rendered files, descriptor ownership, manifests, approved recipe lockfile, generated instructions, workflow ordering, deployment configuration, and final project/state/inference agreement are inspected before promotion. The five existing portfolio/site fixtures remain byte-identical. [`generatedFixtureContracts`](../../scripts/verify-generated-skeletons.mjs) owns the exact fixed inventory and identities; its owning tests bind them to the current descriptors and generated contracts.

New app generation declares `vitest@4.1.11`, matching its existing patched lockfile. Portfolio/site generation and historical transitions retain the `4.1.10` declaration with the approved workspace override to `4.1.11`. Inference recognizes exactly those two declarations for the existing standards Vitest probe; recorded surface fingerprints remain exact, and other package probes remain unchanged. Incoming app transitions preserve the source declaration and its resolved dependency graph. This declaration correction changes no historical descriptor, recipe, lockfile, or approved visual input record.
