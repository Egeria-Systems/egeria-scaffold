...
The certification admission gate is actual and requires one current subject-bound record for every executable descriptor plus repository-present, capability/subject/revision/outcome-bound evidence artifacts. It verifies each evidence-producing revision as an ancestor of the checked candidate and rejects receipts that are incomplete, unresolved, negatively reviewed, or missing affirmative review of the recorded outcome. The current registry marks `booking-calendly@0.1.0` certified from compiled-CLI fresh-scaffold evidence plus one exact-revision protected-staging, provider-confirmed, cancellation, and cleanup journey; current transition closure accepts that record. The six accepted pre-foundation identifier/version/digest subjects remain explicitly `backfill-pending` under the bounded transition policy, so all-certified closure still rejects them. Recurring browser checks remain stubbed and do not establish ongoing Calendly availability. The certification does not prove visual quality, human or assistive-technology usability, general production readiness, or WCAG conformance. Existing-repository changes, transactional migrations, later profiles/capabilities, and unapproved provider actions remain outside the accepted baseline.
