...
**Status:** Controlling architecture summary for the approved P1 builder kernel and in-progress P2 portfolio, including the implemented local Calendly certification foundation awaiting review
...
The certification admission gate is actual and requires one current subject-bound record for every executable descriptor plus repository-present, capability/subject/revision/outcome-bound evidence artifacts. The current registry marks `booking-calendly` pending with actual fresh-scaffold evidence from compiled-CLI output; current closure rejects that pending record. The six accepted pre-foundation identifier/version/digest subjects remain explicitly `backfill-pending` under the bounded transition policy. The manual protected-staging workflow and content-safe provider receipt are prepared, but protected-staging and provider-confirmed outcomes are unexecuted and separately authorized. None of this proves Calendly availability, a real booking, hosted workflow success, deployed behavior, visual quality, human or assistive-technology usability, production readiness, or WCAG conformance. Existing-repository changes, transactional migrations, later profiles/capabilities, and provider actions remain outside the accepted baseline.
