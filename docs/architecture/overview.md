...
Certification uses two actual gates. Clean-checkout admission requires one current subject-bound tracked registry record for every executable descriptor and validates exact subjects, evidence kinds, statuses, and closure policy without reading ignored private artifacts. Before a certification workflow updates that registry, the separately invoked local private-artifact gate requires each referenced plan and capability/subject/revision/outcome-bound receipt, verifies each evidence-producing revision as an ancestor of the checked candidate, and rejects receipts that are incomplete, unresolved, negatively reviewed, or missing affirmative review of the recorded outcome. The current registry marks `booking-calendly@0.1.0` and exact `standards@0.3.0` certified, exact `observability@0.3.0` pending with empty evidence, and four unchanged pre-foundation subjects `backfill-pending`. Observability has accepted local `fresh-scaffold` evidence at `bec51540c9ff4b68af790e9413cd9d1102d54396`; `cleanup-recovery` and `deployed-application` remain future evidence. Descriptor admission passes, while transition closure rejects pending observability and all-certified closure also rejects the four frozen backfills. Recurring browser checks do not establish provider availability. Certification status does not prove visual quality, human or assistive-technology usability, general production readiness, privacy completeness, retained-resource safety, cleanup, recovery, or WCAG conformance. Existing-repository changes, transactional migrations, later profiles/capabilities, and unapproved provider actions remain outside the accepted baseline.
