...
All user-visible or translatable copy originates from validated content or localization files. The generated source registry currently binds `hero`, `text`, `project-list`, and `call-to-action` content schemas to pure presentation components, profile support, variants, accessibility requirements, analytics declarations, and migration hooks. Content may configure only those registered typed sections; it cannot inject executable JSX, JavaScript, CSS, imports, or arbitrary component trees.
