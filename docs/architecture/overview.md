...
The certification admission gate is actual and requires one current subject-bound record for every executable descriptor plus repository-present, capability/subject/revision/outcome-bound evidence artifacts. It verifies each evidence-producing revision as an ancestor of the checked candidate and rejects receipts that are incomplete, unresolved, negatively reviewed, or missing affirmative review of the recorded outcome. The current registry marks `booking-calendly@0.1.0` and the exact materially changed `standards@0.3.0` subject certified, four unchanged pre-foundation subjects `backfill-pending`, and materially changed `observability@0.2.0` ordinary `pending` with its separate present certification plan. Observability still requires cleanup-recovery and deployed-application outcomes in addition to its reviewed fresh-scaffold entry. Transition and all-certified closure therefore remain open for observability; all-certified closure also retains the backfills. Admission still passes. Plan A changes no descriptor subject or certification status; its post-receipt workflow, preview, fixture, and build-order bytes require renewed Task 6D evidence only after an accepted Plan A revision exists. Recurring browser checks do not establish provider availability. Certification status does not prove visual quality, human or assistive-technology usability, general production readiness, or WCAG conformance. Existing-repository changes, transactional migrations, later profiles/capabilities, and unapproved provider actions remain outside the accepted baseline.
