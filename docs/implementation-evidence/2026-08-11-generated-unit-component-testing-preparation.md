...
- [Testing Library setup](https://testing-library.com/docs/react-testing-library/setup/) supports explicit cleanup when test globals are disabled. [user-event setup](https://testing-library.com/docs/user-event/setup/) uses `userEvent.setup()` for interaction state. The generated setup imports `@testing-library/jest-dom/vitest`, the starter component test uses semantic queries, and generated guidance reserves `userEvent.setup()` for tests that add interaction.
