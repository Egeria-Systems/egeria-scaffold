...
Run `pnpm run verify:builder-kernel` for the complete approved builder kernel and current client-ready portfolio increments, including deterministic fixture regeneration and isolated fresh-install/lint/typecheck/Next/OpenNext verification, explicit Chromium installation, and development/workerd Playwright suites for both generated profiles. This command performs public-registry reads, local builds, and local server/browser execution; it does not deploy, call a supplied deployed URL, publish, mutate a provider, prove hosted CI, assess visual quality, or establish accessibility conformance.
