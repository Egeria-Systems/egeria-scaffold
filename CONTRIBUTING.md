...
The current package topology is a private executable `apps/cli`, private `packages/builder-core`, public replaceable `packages/standards`, and public replaceable `packages/observability`. The exact APIs, consumers, and publication guards belong to [package ownership](docs/architecture/package-ownership.md). The builder-kernel candidate also owns exact committed portfolio/site fixtures and a fixed-root isolated verifier; it remains in review until its final review packet receives verified-final-diff approval.
...
Run `pnpm run verify:builder-kernel` for the complete current builder-kernel candidate, including deterministic fixture regeneration and isolated fresh-install/lint/typecheck/Next/OpenNext verification. This command performs public-registry reads and local builds but does not preview, deploy, publish, or mutate a provider.
