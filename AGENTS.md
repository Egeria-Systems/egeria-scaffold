...
- Profiles are versioned materialized recipes. Installed capabilities become authoritative immediately after generation. [ADR-0015](docs/adr/0015-vitest-five-generation.md) owns Vitest 5 generation and retained Vitest 4 compatibility; select installed snapshots independently of new-generation defaults.
- `portfolio` is a one-page public presence; `site` is a multi-page public website; `app@0.2.0` materializes exact `app-foundation@0.1.0` plus `site-routing@0.4.0` by default so it retains the complete production-site experience.
...
- Builder-core supports `app@0.2.0` project/state parsing, rendering, and state-last new-directory generation. The generated app pins `effect@4.0.0-rc.112` directly and confines Effect to application, infrastructure, composition, and delivery server modules. Builder-core, CLI, domain, presentation, client, and content code remain Effect-free. The [capability model](docs/architecture/capability-model.md#accepted-app-architecture-boundary) owns app verification, exact optional-capability addition/removal, and the CLI, transition, immutable-fixture, and remaining certification boundaries. Lifecycle execution retains its own Git, approval, ownership, reference, verification, and final-diff preconditions.
- [ADR-0016](docs/adr/0016-optional-application-persistence.md) owns optional application persistence and its finite shared descriptor versions; the [capability model](docs/architecture/capability-model.md#application-persistence-boundary) owns its lifecycle and certification limits.
- [ADR-0017](docs/adr/0017-transactional-email-resend.md) owns optional transactional email and broader current-profile foundation selection; the [email boundary](docs/architecture/capability-model.md#transactional-email-boundary) owns exact lifecycle and certification limits.
...
- Direct adoption or use of `fast-check` is planned only with the later state/migration invariants that justify property testing. Audited transitive dependencies of a separately approved package do not authorize direct imports or property testing. Workers Vitest is planned only when a capability owns concrete Workers bindings; do not introduce either runtime early.
...
- Use `pnpm run verify:builder-kernel` for the complete current builder-kernel candidate. Its committed portfolio/site/app fixtures are immutable inputs; the fixed-root verifier installs and builds only identity-bounded temporary copies.
