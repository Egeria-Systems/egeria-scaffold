...
- Use `pnpm run verify:generated-visuals` for the opt-in retained-project OpenNext/workerd screenshot comparisons. CI never updates baselines. A baseline update requires a causal source change, the exact pinned Linux/Chromium command documented in the generated README, human diff review, and a second flag-free comparison.
