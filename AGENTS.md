...
- `portfolio` is a one-page public presence; `site` is a multi-page public website; the approved future public `app@0.1.0` recipe materializes exact `app-foundation@0.1.0` plus `site-routing@0.4.0` by default so it retains the complete production-site experience.
...
- The contract-only `app@0.1.0` recipe is implemented. The future generated app pins `effect@4.0.0-rc.112` directly and confines Effect to application, infrastructure, composition, and delivery server modules. Builder-core, CLI, domain, presentation, client, and content code remain Effect-free; dependency installation, runtime, generated surfaces, transitions, fixtures, and certification execution are unimplemented until their separately approved increments.
