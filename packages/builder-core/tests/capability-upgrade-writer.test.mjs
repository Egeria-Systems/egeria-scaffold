...
async function runWriterWithFileSizeLimit(root) {
  ...
  const script = `
    import { createFileSystemCapabilityUpgradeWriter } from ${JSON.stringify(writerModuleUrl)};
    const result = await createFileSystemCapabilityUpgradeWriter(${JSON.stringify(root)}).write([{
      path: "target.txt",
...
test("upgrade writer commits ordered replacements and creates with exact bytes and modes", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-writer-");
  ...
  assert.equal(
    (await readdir(root)).some((name) => name.startsWith(".egeria-upgrade-")),
    false,
  );
});

test("upgrade writer refuses invalid batches and unsafe filesystem state before mutation", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-preflight-");
  ...
  assert.deepEqual(
    await createFileSystemCapabilityUpgradeWriter(rootLink).write([replace]),
    { ok: false, sourceChanged: false },
  );
  assert.deepEqual(
    await createFileSystemCapabilityUpgradeWriter("relative-root").write([
      replace,
    ]),
    { ok: false, sourceChanged: false },
  );
...
test("upgrade writer binds the original root identity", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-root-");
...
test("upgrade writer preflights the complete batch before its first mutation", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-batch-");
...
test("upgrade writer distinguishes failure before the first commit from a retained committed prefix", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-prefix-");
  ...
  const beforeFirst = createFileSystemCapabilityUpgradeWriter(root, {
  ...
  const afterFirst = createFileSystemCapabilityUpgradeWriter(root, {
    beforeCommit: (path) =>
      path === "second.txt"
...
test("upgrade writer refuses replacement and create races without overwriting concurrent bytes", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-race-");
  const replacementPath = join(root, "replace.txt");
  const createPath = join(root, "create.txt");
  await writeFile(replacementPath, "before\n", "utf8");

  const replacementWriter = createFileSystemCapabilityUpgradeWriter(root, {
  ...
  const createWriter = createFileSystemCapabilityUpgradeWriter(root, {
...
test("upgrade writer refuses staged-byte substitution before commit", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-stage-race-");
  const target = join(root, "target.txt");
  await writeFile(target, "before\n", "utf8");
  ...
  const writer = createFileSystemCapabilityUpgradeWriter(root, {
    beforeCommit: async () => {
      const temporaryName = (await readdir(root)).find((name) =>
        name.startsWith(".egeria-upgrade-"),
      );
...
test(
  "upgrade writer retains an actual staging write failure for inspection",
...
test("upgrade writer retains uncertain staged and ancestor-race prefixes for inspection", async (context) => {
  const root = await createTemporaryRoot(context, "egeria-upgrade-staging-");
  const target = join(root, "target.txt");
  await writeFile(target, "before\n", "utf8");
  let changedTemporaryPath;
  const stagedWriter = createFileSystemCapabilityUpgradeWriter(root, {
    beforeCommit: async () => {
      const temporaryName = (await readdir(root)).find((name) =>
        name.startsWith(".egeria-upgrade-"),
      );
  ...
  const outside = await createTemporaryRoot(context, "egeria-upgrade-outside-");
  ...
  const ancestorRaceWriter = createFileSystemCapabilityUpgradeWriter(root, {
    beforeCommit: async () => {
