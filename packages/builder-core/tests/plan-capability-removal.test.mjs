...
async function appControlEntries() {
  const source = await baseFixtureEntries("site");
  ...
  const resolved = core.resolveCapabilities({
    profile: "app",
    requestedCapabilities: ["booking-calendly", "multilingual", "analytics"],
  }, catalog.value, core.profileRecipes);
  ...
  return new Map([
    [".egeria/project.yaml", JSON.stringify({
      ...
      capabilitySettings: { "booking-calendly": settings, analytics: analyticsSettings },
    })],
    [".egeria/state.json", JSON.stringify({
      ...
      lastSuccessfulVerification: {
        kind: "generation",
        checks: [...generationChecks.slice(0, -1), "worker-integration", "post-state-inference"],
      },
    })],
...
test("staged app boundary refuses capability removal and forged approval before effects", async () => {
  const entries = await appControlEntries();
  for (const capability of ["booking-calendly", "multilingual", "analytics"]) {
    assertFailure(await planFromEntries(entries, { capability }), "CAPABILITY_REMOVAL_UNSUPPORTED");
    ...
    const applied = await core.applyCapabilityRemoval({
      ...
      inspectRepositoryInventory: forbiddenEffect,
      now: forbiddenEffect,
    });
