...
export function createHealthRoute(dependencies: HealthRouteDependencies) {
  return async (request: Request): Promise<Response> => {
    ...
    const exit = await Effect.runPromiseExit(
      composeHealth(context, dependencies.readerLayer, dependencies.readRuntimeEnvironment),
    ...
    if (defect === undefined) {
      ...
      if (reasons.length > 0 && reasons.every(
        (reason) => Cause.isFailReason(reason) && reason.error instanceof ApplicationEnvironmentInvalid,
      )) {
        return jsonResponse(503, {
          status: "unavailable",
          requestId: context.requestId,
          error: { code: "application-environment-invalid" },
        });
      }
      if (reasons.length > 0 && reasons.every(
