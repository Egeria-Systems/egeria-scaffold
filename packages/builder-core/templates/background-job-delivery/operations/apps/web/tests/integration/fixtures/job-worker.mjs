...
const syntheticWorker = {
  ...
  async fetch(request, environment, context) {
    ...
    if (pathname === "/__synthetic-jobs/remove-terminal") {
      const input = await request.json();
      if (input.ref !== "synthetic-terminal-0" || terminals.length !== 1) return new Response(null, { status: 409 });
      // The observation consumer already acknowledged the synthetic DLQ copy.
      // This endpoint models only targeted REST removal in the local tool test.
