...
type BookingFrameProperties = Readonly<{
  settings: CalendlyBookingSettings & Readonly<{ destination: string }>;
  copy: BookingContent;
}>;
...
export function CalendlyBooking({ settings, copy }: CalendlyBookingProperties) {
  if (settings.destination === undefined) {
    return (
      ...
      >
        <h2 id="booking-heading" className="text-3xl font-bold tracking-tight">{copy.heading}</h2>
        <p className="mt-4 text-lg text-muted">{copy.unavailable}</p>
      </section>
    );
  }
  const configuredSettings = { ...settings, destination: settings.destination };
  return <ConfiguredCalendlyBooking settings={configuredSettings} copy={copy} />;
}

function ConfiguredCalendlyBooking({ settings, copy }: BookingFrameProperties) {
