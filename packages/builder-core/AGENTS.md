...
`packages/builder-core` is the private owner of builder internals and project/state schemas. P1 Tasks 1 and 2 added the executable contracts, checked JSON Schema artifacts, exact six-capability catalog, `portfolio` and `site` recipes, deterministic resolution, and installed-manifest projection. P1 Task 3 added strict `.egeria` codecs plus pure hybrid-ownership fingerprint materialization. P1 Task 4 added fixed-root read-only repository access and deterministic capability and surface evidence. P1 Task 5 composes those owners into content-safe read-only diagnostics and structural differences.
...
- Keep inference evidence separate from policy. Task 5 diagnostics compose existing codecs, resolution, and inference; they neither authorize nor perform a repository change.
