...
type PreparedChange = Readonly<{
  change: CapabilityUpgradeFileChange;
  target: string;
  parent?: PathIdentity;
...
async function prepareChange(
  ...
  if (change.expected.kind === "missing") {
    ...
    try {
      ...
      return error instanceof Error && "code" in error && error.code === "ENOENT"
        ? { change, target, parent: parent.identity }
        : undefined;
    }
  }
  ...
  return identity !== undefined &&
    ...
    ? { change, target, parent: confirmedParent.identity, identity }
    : undefined;
}

function validChange(value: unknown): value is CapabilityUpgradeFileChange {
  ...
  return (
    ...
    (initial.parent === undefined ||
      (current.parent !== undefined &&
        samePathIdentity(initial.parent, current.parent)))
  );
}
...
async function writePreparedChange(
  ...
  if (
    ...
    current.parent === undefined ||
    !samePathIdentity(parentResult.parent, current.parent)
  ) {
    return failBeforeInstall();
  }
