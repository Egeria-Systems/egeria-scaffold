...
The implemented exact Calendly addition/removal boundaries and the first read-only exact standards upgrade planner leave upgrade execution, every other supported edge or removal executor, generic transforms, provider actions, persistent-data cleanup, and automated recovery deferred.
...
- `apps/cli` is constrained to nine exact commands for this approved increment: `create`, `infer`, `doctor`, `diff`, read-only `plan-add`, fingerprint-gated `apply-add`, read-only `plan-remove`, fingerprint-gated `apply-remove`, and read-only `plan-upgrade`. `apply-remove` is the exact current removal command; its successful result is transaction evidence awaiting verified-final-diff approval.
- `packages/builder-core` is private and owns the current runtime contracts, checked schemas, exact seven-capability catalog plus the exact historical standards upgrade-source snapshot, recipes, deterministic resolution, state inspection, diagnostics, bounded Git preflight and exact-diff inspection, redacted addition/removal/upgrade planning, the exact state-last Calendly addition/removal transactions, allowlisted template rendering, and state-last new-directory generation.
