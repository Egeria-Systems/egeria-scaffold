...
The current implementation advances that accepted baseline with the exact fingerprint-gated Calendly addition transaction and read-only Calendly removal planning while leaving removal execution and later lifecycle work deferred.
...
The implemented addition boundary leaves upgrades, removals, generic transforms, provider actions, and automated recovery deferred.
...
- `apps/cli` is constrained to eight exact commands for this approved increment: `create`, `infer`, `doctor`, `diff`, read-only `plan-add`, fingerprint-gated `apply-add`, read-only `plan-remove`, and planned fingerprint-gated `apply-remove`. `apply-remove` is the approved next command and is not represented as implemented until its transaction and compiled lifecycle evidence pass.
- `packages/builder-core` is private and owns the current runtime contracts, checked schemas, exact seven-capability catalog, recipes, deterministic resolution, state inspection, diagnostics, bounded Git preflight and exact-diff inspection, redacted addition/removal planning, the exact state-last Calendly addition transaction, allowlisted template rendering, and state-last new-directory generation.
