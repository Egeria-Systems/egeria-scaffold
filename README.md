...
The implemented exact Calendly addition/removal boundaries leave upgrades, every other removal executor, generic transforms, provider actions, persistent-data cleanup, and automated recovery deferred.
...
- `apps/cli` is constrained to eight exact commands for this approved increment: `create`, `infer`, `doctor`, `diff`, read-only `plan-add`, fingerprint-gated `apply-add`, read-only `plan-remove`, and fingerprint-gated `apply-remove`. `apply-remove` is the exact current removal command; its successful result is transaction evidence awaiting verified-final-diff approval.
- `packages/builder-core` is private and owns the current runtime contracts, checked schemas, exact seven-capability catalog, recipes, deterministic resolution, state inspection, diagnostics, bounded Git preflight and exact-diff inspection, redacted addition/removal planning, the exact state-last Calendly addition/removal transactions, allowlisted template rendering, and state-last new-directory generation.
