...
- `apps/cli` is a private executable adapter for seven exact commands: `create`, `infer`, `doctor`, `diff`, read-only `plan-add`, fingerprint-gated `apply-add`, and read-only `plan-remove`. Addition accepts exact paired Calendly settings, application additionally requires the approved plan fingerprint, and removal planning accepts only the exact capability identifier.
