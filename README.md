...
Generated visual regression preserves the automated accessibility suite and adds four exact-pixel OpenNext/workerd comparisons: desktop and 320-pixel responsive states for each of the portfolio and site profiles. `pnpm run verify:generated-visuals` runs all retained-project verification plus those comparisons in identity-bounded copies; CI uses the exact digest-pinned Linux/Chromium image and never updates snapshots. Generated CI runs the visual comparison only after its prepared-preview check succeeds so an earlier browser failure keeps its diagnostics. The root verifier exports visual-failure Playwright reports and test results before deleting its isolated copy, and the repository workflow retains available expected, actual, and diff images for seven days. Screenshot equality is narrow regression evidence, not a visual quality, human accessibility, deployed-behavior, production-readiness, or WCAG conformance claim. Performance budgets remain the separate immediately following increment and are not implemented.
