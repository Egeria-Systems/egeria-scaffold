...
`plan-add` treats tracked changes, all non-ignored untracked files, and the `assume-unchanged` and `skip-worktree` index flags as dirty. It requires installed application-owned paths to remain files and refuses managed drift, ejections, present or ignored create-target collisions, Git operations/conflicts, primary or detached worktrees, unexpected Git/reader failures, and identity changes between its two inspections. It redacts settings and branch/root/Git-directory/dirty-path metadata while intentionally retaining the exact base revision. Its result is local eligibility and planning evidence, not transformation, migration, recovery, certification, deployment, provider behavior, performance, visual quality, accessibility conformance, security clearance, production readiness, or real-client evidence. The command creates no branch or worktree and performs no writes.
