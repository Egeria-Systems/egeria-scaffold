...
- `apps/cli` is a private executable adapter for six exact commands: `create`, `infer`, `doctor`, `diff`, read-only `plan-add`, and fingerprint-gated `apply-add`. The Calendly commands accept their exact paired settings, and application additionally requires the approved plan fingerprint.
- `packages/builder-core` is private and owns the current runtime contracts, checked schemas, exact seven-capability catalog, recipes, deterministic resolution, state inspection, diagnostics, bounded Git preflight and exact-diff inspection, redacted addition planning, the exact state-last Calendly addition transaction, allowlisted template rendering, and state-last new-directory generation.
