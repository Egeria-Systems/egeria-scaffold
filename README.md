...
`plan-remove` is read-only for exact installed `booking-calendly@0.1.0` in current `portfolio` and `site` recipes. It requires exact control, desired/installed/inferred, inventory, and ejection agreement; refuses managed and shared-route drift; classifies unchanged capability files for deletion and modified/already-ejected application files for `preserve-file-and-eject`; and returns exact `CAPABILITY_NOT_INSTALLED` for a clean repository without the capability. Its path-sorted public plan is redacted while the fingerprint binds the full private project, installed state, actions, and Git identity. It does not execute any action or persist state.
