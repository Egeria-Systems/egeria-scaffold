...
`plan-add` treats tracked changes and all non-ignored untracked files as dirty, refuses current drift/customization, ejections, collisions, Git operations/conflicts, primary or detached worktrees, and identity changes between its two inspections, and redacts settings and repository metadata. Its result is local eligibility and planning evidence, not transformation, migration, recovery, certification, deployment, provider behavior, performance, visual quality, accessibility conformance, security clearance, production readiness, or real-client evidence. The command creates no branch or worktree and performs no writes.
