...
- `apps/cli` is a private executable adapter for five exact commands: `create`, `infer`, `doctor`, `diff`, and read-only `plan-add`. `create` and `plan-add` accept their exact paired Calendly arguments.
- `packages/builder-core` is private and owns the current runtime contracts, checked schemas, exact seven-capability catalog, recipes, deterministic resolution, state inspection, diagnostics, bounded Git preflight, redacted addition planning, allowlisted template rendering, and state-last new-directory generation.
...
Calendly is an explicit optional selection for initial `portfolio` or `site` creation; it is not a recipe default. A clean attached linked worktree can now receive a redacted approval-required plan for later addition, but the builder cannot execute that plan. The paired CLI options accept `link`, `inline`, or `popup` plus a bounded HTTPS destination on `calendly.com` or `www.calendly.com` with no query string. Selection and strict settings are recorded together in `.egeria/project.yaml` only during initial generation; the installed capability and managed-surface fingerprints are recorded in `.egeria/state.json` and checked by read-only inference.

`plan-add` treats tracked changes, all non-ignored untracked files, and the `assume-unchanged` and `skip-worktree` index flags as dirty. It requires the installed state to retain the exact canonical managed-surface inventory and installed application-owned paths to remain files, while ignoring application-owned content fingerprints. It refuses managed drift, ejections, present or ignored create-target collisions, Git operations/conflicts, primary or detached worktrees, unexpected Git/reader failures, and identity changes between its two inspections. It redacts settings and branch/root/Git-directory/dirty-path metadata while intentionally retaining the exact base revision. Its result is local eligibility and planning evidence, not transformation, migration, recovery, certification, deployment, provider behavior, performance, visual quality, accessibility conformance, security clearance, production readiness, or real-client evidence. The command creates no branch or worktree and performs no writes.
